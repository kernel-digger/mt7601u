...
{
	hex_dump("RxFCEInfo Raw Data", (UCHAR *) pRxFceInfo,
		 sizeof(RXFCE_INFO));
...
static UCHAR *txwi_txop_str[] =
    { "HT_TXOP", "PIFS", "SIFS", "BACKOFF", "Invalid" };
...
VOID dump_rxwi(RTMP_ADAPTER * pAd, RXWI_STRUC * pRxWI)
{
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("\tPhyMode=%d(%s)\n", pRxWI->RxWIPhyMode,
		  get_phymode_str(pRxWI->RxWIPhyMode)));
#ifdef RT65xx
	if (IS_RT65XX(pAd) && (pRxWI->RxWIPhyMode == MODE_VHT))
		DBGPRINT(RT_DEBUG_OFF,
			 ("\tMCS=%d(Nss:%d, MCS:%d)\n", pRxWI->RxWIMCS,
			  (pRxWI->RxWIMCS >> 4), (pRxWI->RxWIMCS & 0xf)));
	else
#endif /* RT65xx */
		DBGPRINT(RT_DEBUG_OFF, ("\tMCS=%d\n", pRxWI->RxWIMCS));
...
static UCHAR *txinfo_que_str[] =
    { "MGMT", "HCCA", "EDCA_1", "EDCA_2", "Invalid" };
...
#ifdef RLT_MAC
	{
		struct _TXINFO_NMAC_PKT *pkt_txinfo =
		    (struct _TXINFO_NMAC_PKT *)pTxInfo;
		...
		DBGPRINT(RT_DEBUG_OFF,
			 ("\td_port=%d(%s)\n", pkt_txinfo->d_port,
			  TXINFO_DPORT_STR(pkt_txinfo->d_port)));
		DBGPRINT(RT_DEBUG_OFF,
			 ("\tQSEL=%d(%s)\n", pkt_txinfo->QSEL,
			  TXINFO_QUE_STR(pkt_txinfo->QSEL)));
...
static VOID dumpTxBlk(TX_BLK * pTxBlk)
{
	...
	for (i = 0; i < frameNum; i++) {
		...
		if (pPacket) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("\t\t[%d]:ptr=0x%x, Len=%d!\n", i,
				  (UINT32) (GET_OS_PKT_DATAPTR(pPacket)),
			      ...
			      IN UCHAR QueIdx, IN UCHAR * pData, IN UINT Length)
{
	...
	do {
		...
		if ((FreeNum > 0)) {
			...
			Status =
			    MlmeHardTransmit(pAd, QueIdx, pPacket,
					     FlgDataQForce, FlgIsLocked);
			if (Status == NDIS_STATUS_SUCCESS)
				retryCnt = 0;
			else
				     ...
				     IN UCHAR QueIdx, IN PNDIS_PACKET pPacket)
{
	...
	pHeader_802_11 =
	    (PHEADER_802_11) (pSrcBufVA + TXINFO_SIZE + TSO_SIZE + TXWISize);
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (WMODE_EQUAL
		    (pAd->CommonCfg.PhyMode, WMODE_A | WMODE_B | WMODE_G)
#ifdef DOT11_N_SUPPORT
		    || WMODE_EQUAL(pAd->CommonCfg.PhyMode,
				   WMODE_A | WMODE_B | WMODE_G | WMODE_AN |
				   WMODE_GN)
...
//+++Add by shiang for debug
	if (0) {
		hex_dump("TxMgmtPkt", (UCHAR *) pHeader_802_11,
			 ((SrcBufLen - TXINFO_SIZE - TXWISize - TSO_SIZE) >
			  7000 ? 7000 : (SrcBufLen - TXINFO_SIZE - TXWISize -
					 TSO_SIZE)));
	}
//---Add by shiang for debug
...
static UCHAR TxPktClassification(IN RTMP_ADAPTER * pAd, IN PNDIS_PACKET pPacket)
{
	...
	if (RTMP_GET_PACKET_LOWRATE(pPacket)) {	/* It's a specific packet need to force low rate, i.e., bDHCPFrame, bEAPOLFrame, bWAIFrame */
		...
		if (RTMP_GET_PACKET_MOREDATA(pPacket)
			...
			TxFrameType = TX_LEGACY_FRAME;
#endif /* UAPSD_SUPPORT */
		else if (((pMacEntry->TXBAbitmap &
			   (1 << (RTMP_GET_PACKET_UP(pPacket)))) != 0)
#ifdef WFA_VHT_PF
		...
		if ((CLIENT_STATUS_TEST_FLAG
		     (pMacEntry, fCLIENT_STATUS_AGGREGATION_CAPABLE)
	...
	if ((RTMP_GET_PACKET_FRAGMENTS(pPacket) > 1)
	    && (TxFrameType == TX_LEGACY_FRAME)
#ifdef DOT11_N_SUPPORT
	    && ((pMacEntry->TXBAbitmap & (1 << (RTMP_GET_PACKET_UP(pPacket))))
		== 0)
#endif /* DOT11_N_SUPPORT */
	    )
		       ...
		       IN UCHAR QIdx, IN INT Max_Tx_Packets)
{
	...
	for (QueIdx = sQIdx; QueIdx <= eQIdx; QueIdx++) {
		...
		while (1) {
...
#ifdef CONFIG_MULTI_CHANNEL
			if ((pAd->MultiChannelFlowCtl & (1 << QueIdx)) ==
			    (1 << QueIdx)) {
				break;
			}
#endif /* CONFIG_MULTI_CHANNEL */
...
#ifdef DBG_DIAGNOSE
				if (firstRound == TRUE)
					pDiagStruct->TxSWQueCnt[pDiagStruct->
								ArrayCurIdx]
					    [0]++;
#endif /* DBG_DIAGNOSE */
			...
			if (pTxBlk->TxFrameType &
			    (TX_RALINK_FRAME | TX_AMSDU_FRAME)) {
				...
				IN PNDIS_PACKET pPacket, IN UCHAR OpMode)
{
	...
	nMSDU =
	    deaggregate_AMSDU_announce(pAd, pPacket, pData, DataSize, OpMode);
			   ...
			   OUT PUCHAR pUserPriority, OUT PUCHAR pQueIdx)
{
	 ...
	 */
	bWmmReq = (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_WMM_INUSED))
	    && ((pMacEntry) && ((VALID_WCID(pMacEntry->Aid)
				 && CLIENT_STATUS_TEST_FLAG(pMacEntry,
							    fCLIENT_STATUS_WMM_CAPABLE))
	...
	if (TypeLen == 0x8100) {
		...
		if (bWmmReq) {
			...
			*pQueIdx =
			    MapUserPriorityToAccessCategory[*pUserPriority];
		}
			...
			IN RSSI_SAMPLE * pRssi, IN RXWI_STRUC * pRxWI)
{
	...
	if (rssi[0] != 0) {
		pRssi->LastRssi0 =
		    ConvertToRssi(pAd, (CHAR) rssi[0], RSSI_0, pRxWI->RxWISNR1,
				  pRxWI->RxWIBW);
		if (bInitial) {
	...
	if (rssi[1] != 0) {
		pRssi->LastRssi1 =
		    ConvertToRssi(pAd, (CHAR) rssi[1], RSSI_1, pRxWI->RxWISNR1,
				  pRxWI->RxWIBW);
		if (bInitial) {
			...
			pRssi->AvgRssi1X8 =
			    (pRssi->AvgRssi1X8 - pRssi->AvgRssi1) +
			    pRssi->LastRssi1;
	...
	if (rssi[2] != 0) {
		pRssi->LastRssi2 =
		    ConvertToRssi(pAd, (CHAR) rssi[2], RSSI_2, pRxWI->RxWISNR1,
				  pRxWI->RxWIBW);
		...
		if (bInitial) {
			...
			pRssi->AvgRssi2X8 =
			    (pRssi->AvgRssi2X8 - pRssi->AvgRssi2) +
			    pRssi->LastRssi2;
...
VOID Indicate_Legacy_Packet(IN RTMP_ADAPTER * pAd,
			    IN RX_BLK * pRxBlk, IN UCHAR FromWhichBSSID)
{
...
//+++Add by shiang for debug
	if (0) {
		hex_dump("Indicate_Legacy_Packet", pRxBlk->pData,
			 pRxBlk->DataSize);
		hex_dump("802_11_hdr", (UCHAR *) pRxBlk->pHeader,
			 LENGTH_802_11);
	}
//---Add by shiang for debug
...
//+++Add by shiang for debug
	if (0) {
		hex_dump("Before80211_2_8023", pRxBlk->pData,
			 pRxBlk->TransDataSize);
		hex_dump("header802_3", &Header802_3[0], LENGTH_802_3);
	}
//---Add by shiang for debug

	RtmpOsSetPacket(get_netdev_from_bssid(pAd, FromWhichBSSID), pRxPacket,
...
#endif /* DOT11_N_SUPPORT */
		{
			Indicate_Legacy_Packet_Hdr_Trns(pAd, pRxBlk,
							FromWhichBSSID);
		}
	}
}
...
#ifdef CONFIG_STA_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		    ANNOUNCE_OR_FORWARD_802_3_PACKET(pAd, pPacket2,
						     FromWhichBSSID);
#endif /* CONFIG_STA_SUPPORT */
	}
}
...
VOID Indicate_EAPOL_Packet(IN RTMP_ADAPTER * pAd,
			   IN RX_BLK * pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	if (pRxBlk->pRxWI->RxWIWirelessCliID >= MAX_LEN_OF_MAC_TABLE) {
		DBGPRINT(RT_DEBUG_WARN,
			 ("Indicate_EAPOL_Packet: invalid wcid.\n"));
		RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket,
				    NDIS_STATUS_FAILURE);
		return;
	}
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		{
			ASSERT((pRxBlk->pRxWI->RxWIWirelessCliID ==
				BSSID_WCID));
...
VOID ReSyncBeaconTime(IN RTMP_ADAPTER * pAd)
{
	UINT32 Offset;
				 ...
				 IN PTX_BLK pTxBlk, IN PUCHAR pHdr)
{
	    ...
	    CLIENT_STATUS_TEST_FLAG(pMacEntry, fCLIENT_STATUS_SOFTWARE_ENCRYPT))
	{
		...
		pTxBlk->pKey = pKey;
		if ((pKey->CipherAlg == CIPHER_WEP64)
		    || (pKey->CipherAlg == CIPHER_WEP128))
			inc_iv_byte(pKey->TxTsc, LEN_WEP_TSC, 1);
		else if ((pKey->CipherAlg == CIPHER_TKIP)
			  ...
			  IN BOOLEAN bQosNull, IN BOOLEAN bEOSP, IN UCHAR OldUP)
{
	...
	if (NState == NDIS_STATUS_SUCCESS) {
		...
		MiniportMMRequest(pAd, MapUserPriorityToAccessCategory[7],
				  (PUCHAR) pNullFr, Length);
...
BOOLEAN CmdRspEventCallbackHandle(PRTMP_ADAPTER pAd, PUCHAR pRspBuffer)
{
	...
	if ((pFceInfo->info_type != CMD_PACKET)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Packet is not command response/event\n"));
		return NDIS_STATUS_FAILURE;
	}
	...
	if (pFceInfo->self_gen) {
		...
		DlListForEachSafe(CmdRspEvent, CmdRspEventTmp,
				  &MCtrl->CmdRspEventList, struct CMD_RSP_EVENT,
				  List) {
			if (CmdRspEvent->CmdSeq == pFceInfo->cmd_seq) {

				if ((*CmdRspEvent->RspPayloadLen ==
				     pFceInfo->pkt_len)
				    && (*CmdRspEvent->RspPayloadLen != 0)) {
					NdisMoveMemory(*CmdRspEvent->RspPayload,
						       pRspBuffer +
						       sizeof(*pFceInfo),
						       pFceInfo->pkt_len);
				} else if ((*CmdRspEvent->RspPayloadLen == 0)
					   && (pFceInfo->pkt_len == 8)) {
					DBGPRINT(RT_DEBUG_INFO,
...
NTSTATUS StopDmaRx(IN RTMP_ADAPTER * pAd, IN UCHAR Level)
{
	...
	while (1) {
		...
		pRxBlk = &RxBlk;
		pRxPacket =
		    GetPacketFromRxRing(pAd, pRxBlk, &bReschedule, &RxPending);
