	...
	if ((SetMask & ALLN_SETPROTECT) != 0) {
		...
		case 0x0:
			...
			if (IS_RT65XX(pAd)) {
				...
				RTMP_IO_READ32(pAd, TX_PROT_CFG6,
					       &vht_port_cfg.word);
				vht_port_cfg.field.ProtectCtrl = 0;
				RTMP_IO_WRITE32(pAd, TX_PROT_CFG6,
						vht_port_cfg.word);

				RTMP_IO_READ32(pAd, TX_PROT_CFG7,
					       &vht_port_cfg.word);
				vht_port_cfg.field.ProtectCtrl = 0;
				RTMP_IO_WRITE32(pAd, TX_PROT_CFG7,
						vht_port_cfg.word);

				RTMP_IO_READ32(pAd, TX_PROT_CFG8,
					       &vht_port_cfg.word);
				vht_port_cfg.field.ProtectCtrl = 0;
				RTMP_IO_WRITE32(pAd, TX_PROT_CFG8,
						vht_port_cfg.word);
			}
#endif /* RT65xx */
#endif /* DOT11_VHT_AC */
...
/*
	...
	==========================================================================
 */
VOID AsicSwitchChannel(IN RTMP_ADAPTER * pAd,
		       IN UCHAR Channel, IN BOOLEAN bScan)
{
	...
	if ((RTMP_Usb_AutoPM_Get_Interface(pObj->pUsb_Dev, pObj->intf)) == 1) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s(): autopm_resume success\n", __FUNCTION__));
		RTMP_CLEAR_FLAG(pAd, fRTMP_ADAPTER_SUSPEND);
	} else if ((RTMP_Usb_AutoPM_Get_Interface(pObj->pUsb_Dev, pObj->intf))
		   == (-1)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s(): autopm_resume fail\n", __FUNCTION__));
		RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_SUSPEND);
		return;
	} else
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s(): autopm_resume do nothing\n", __FUNCTION__));
...
VOID InitLookupTable(IN PRTMP_ADAPTER pAd)
{
	...
	UCHAR PlusStepNum[IEEE80211_BAND_NUMS][8] =
	    { {0, 1, 3, 2, 3, 3, 3, 2}, {0, 1, 3, 2, 3, 3, 3, 2} };
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4G] EEPROM 6e = %x\n",
		  WordStruct.word));
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4G] EEPROM 70 = %x\n",
		  WordStruct.word));
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4G] EEPROM 72 = %x\n",
		  WordStruct.word));
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4] EEPROM 74 = %x\n",
		  WordStruct.word));
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4G] EEPROM 76 = %x\n",
		  WordStruct.word));
	pAd->TxPowerCtrl.TssiGain[IEEE80211_BAND_2G] =
	    (WordStruct.field.Byte0 & 0x0F);
	Step[IEEE80211_BAND_2G] = (WordStruct.field.Byte0 >> 4);
	pAd->TxPowerCtrl.RefTemp[IEEE80211_BAND_2G] =
	    (CHAR) WordStruct.field.Byte1;

	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4G] Plus = %u %u %u %u %u %u %u %u\n",
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4G] Minus = %u %u %u %u %u %u %u %u\n",
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4G] tssi gain/step = %u\n",
		  pAd->TxPowerCtrl.TssiGain[IEEE80211_BAND_2G]));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4] Step = %u\n",
		  Step[IEEE80211_BAND_2G]));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation 2.4] RefTemp_2G = %d\n",
		  pAd->TxPowerCtrl.RefTemp[IEEE80211_BAND_2G]));
	...
	if (RFIC_IS_5G_BAND(pAd)) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation 5G] EEPROM d4 = %x\n",
			  WordStruct.word));
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation 5G] EEPROM d8 = %x\n",
			  WordStruct.word));
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation 5G] EEPROM da = %x\n",
			  WordStruct.word));
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation 5G] EEPROM dc = %x\n",
			  WordStruct.word));
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation 5G] Plus = %u %u %u %u %u %u %u %u\n",
			  PlusStepNum[IEEE80211_BAND_5G][0],
			  PlusStepNum[IEEE80211_BAND_5G][1],
			  PlusStepNum[IEEE80211_BAND_5G][2],
			  PlusStepNum[IEEE80211_BAND_5G][3],
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation 5G] Minus = %u %u %u %u %u %u %u %u\n",
			  MinusStepNum[IEEE80211_BAND_5G][0],
			  MinusStepNum[IEEE80211_BAND_5G][1],
			  MinusStepNum[IEEE80211_BAND_5G][2],
			  MinusStepNum[IEEE80211_BAND_5G][3],
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation 5G] tssi gain/step = %u\n",
			  pAd->TxPowerCtrl.TssiGain[IEEE80211_BAND_5G]));
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation 5G] Step = %u\n",
			  Step[IEEE80211_BAND_5G]));
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation 5G] RefTemp_2G = %d\n",
			  pAd->TxPowerCtrl.RefTemp[IEEE80211_BAND_5G]));
	...
	for (band = IEEE80211_BAND_2G; band < band_nums; band++) {
		...
		pAd->TxPowerCtrl.LookupTable[band][0 + Offset] =
		    pAd->TxPowerCtrl.LookupTable[band][1 + Offset] - Step[band];
		for (Idx = 2; Idx < 26;) {	/* Idx++ ) */
			if (PlusStepNum[band][i] != 0 || i >= 8) {
				if (Idx >= IdxTmp + PlusStepNum[band][i]
				    && i < 8) {
					pAd->TxPowerCtrl.LookupTable[band][Idx +
									   Offset]
					    =
					    pAd->
					    TxPowerCtrl.LookupTable[band][Idx -
									  1 +
									  Offset]
					    + (Step[band] - (i + 1) + 1);
		...
		for (Idx = 1; Idx < 8;) {	/* Idx++ ) */
			if (MinusStepNum[band][i] != 0 || i >= 8) {
				if ((Idx + 1) >= IdxTmp + MinusStepNum[band][i]
				    && i < 8) {
					pAd->
					    TxPowerCtrl.LookupTable[band][-Idx +
									  Offset]
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("[temp. compensation] Lookup table as below:\n"));
		for (Idx = 0; Idx < 33; Idx++) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("[temp. compensation band(%d)] %d, %d\n",
				  band, Idx - Offset,
				  pAd->TxPowerCtrl.LookupTable[band][Idx]));
		}
	}
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("[temp. compensation] Set RF_R27 to 0x%x\n", RFValue));
}
					      ...
					      IN PCHAR pDeltaPowerByBbpR1)
{
	...
	UCHAR RFValue = 0;
	CHAR TuningTableUpperBound = 0, TuningTableIndex0 =
	    0, TuningTableIndex1 = 0;
	INT CurrentTemp = 0;
	INT RefTemp;
	INT *LookupTable;
	INT LookupTableIndex =
	    pAd->TxPowerCtrl.LookupTableIndex +
	    TEMPERATURE_COMPENSATION_LOOKUP_TABLE_OFFSET;
	...
	*pTotalDeltaPwr = 0;
	...
	if (pAd->CommonCfg.Channel > 14) {
		...
		LookupTable =
		    &pAd->TxPowerCtrl.LookupTable[IEEE80211_BAND_5G][0];
...
#endif /* A_BAND_SUPPORT */
	{
		...
		LookupTable =
		    &pAd->TxPowerCtrl.LookupTable[IEEE80211_BAND_2G][0];
	...
	if (bAutoTxAgc) {
		...
		if (CurrentTemp >
		    ...
		    ((LookupTable[LookupTableIndex + 1] -
		      LookupTable[LookupTableIndex]) >> 2)
		    && LookupTableIndex < 32) {
			...
			pAd->TxPowerCtrl.LookupTableIndex++;
			   ...
			   ((LookupTable[LookupTableIndex] -
			     LookupTable[LookupTableIndex - 1]) >> 2)
			   && LookupTableIndex > 0) {
			DBGPRINT(RT_DEBUG_INFO, ("[temp. compensation] --\n"));
			LookupTableIndex--;
			pAd->TxPowerCtrl.LookupTableIndex--;
		...
		TuningTableIndex0 =
		    (TuningTableIndex0 >
		     TuningTableUpperBound) ? TuningTableUpperBound :
		    TuningTableIndex0;
		TuningTableIndex0 =
		    (TuningTableIndex0 <
		...
		TxPowerTuningTableEntry0 =
		    &TxPowerTuningTable[TuningTableIndex0 +
		...
		TxPowerTuningTableEntry1 =
		    &TxPowerTuningTable[TuningTableIndex1 +
					TX_POWER_TUNING_ENTRY_OFFSET];
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ("[temp. compensation] (tx1)RF_TX_ALC = %x, MAC_PowerDelta = %d, TuningTableIndex = %d\n",
			  ...
			  TxPowerTuningTableEntry1->MAC_PowerDelta,
			  TuningTableIndex1));

		/* Update RF_R49 [0:5] */
		RT30xxReadRFRegister(pAd, RF_R49, &RFValue);
		RFValue =
		    ((RFValue & ~0x3F) | TxPowerTuningTableEntry0->RF_TX_ALC);
		...
		RFValue =
		    ((RFValue & ~0x3F) | TxPowerTuningTableEntry1->RF_TX_ALC);
		if ((RFValue & 0x3F) > 0x27) {	/* The valid range of the RF R49 (<5:0>tx0_alc<5:0>) is 0x00~0x27 */
			RFValue = ((RFValue & ~0x3F) | 0x27);
		}
		DBGPRINT(RT_DEBUG_INFO,
...
VOID AsicEnableIbssSync(IN PRTMP_ADAPTER pAd)
{
...
#ifdef RT_BIG_ENDIAN
	{
		...
		NdisMoveMemory((PUCHAR) & localTxWI, (PUCHAR) & pAd->BeaconTxWI,
			       TXWISize);
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("--->AsicEnableIbssSync(TxWIMPDUByteCnt=%d, beaconLen=%d)\n",
		  pAd->BeaconTxWI.TxWIMPDUByteCnt, beaconLen));

	DBGPRINT(RT_DEBUG_TRACE,
		 ("--->AsicEnableIbssSync(ADHOC mode. TxWIMPDUByteCnt = %d)\n",
	...
	for (i = 0; i < TXWISize; i += 2) {
		longptr = *ptr + (*(ptr + 1) << 8);
		RTMP_CHIP_UPDATE_BEACON(pAd, HW_BEACON_BASE0(pAd) + i, longptr,
					2);
		ptr += 2;
	}
...
/*#ifdef WIFI_TEST*/
		if (pAd->CommonCfg.bWiFiTest) {
			if (Ac3Cfg.field.AcTxop == 102) {
				Ac0Cfg.field.AcTxop =
				    pEdcaParm->
				    Txop[QID_AC_BE] ? pEdcaParm->Txop[QID_AC_BE]
				    : 10;
...
VOID AsicRemovePairwiseKeyEntry(IN PRTMP_ADAPTER pAd, IN UCHAR Wcid)
{
	...
	AsicUpdateWcidAttributeEntry(pAd,
				     BSS0,
				     0, CIPHER_NONE, Wcid, PAIRWISEKEYTABLE);
...
BOOLEAN AsicSendCommandToMcu(IN RTMP_ADAPTER * pAd,
			     IN UCHAR Command,
			     IN UCHAR Token,
			     IN UCHAR Arg0, IN UCHAR Arg1, IN BOOLEAN in_atomic)
{
#ifdef RT65xx
	...
	if (pAd->CommonCfg.ETxBfEnCond) {
		...
		for (idx = 1; idx < MAX_LEN_OF_MAC_TABLE; idx++) {
			...
			if ((IS_ENTRY_CLIENT(pEntry)) && (pEntry->eTxBfEnCond)) {
				...
				RTMPCancelTimer(&pEntry->eTxBfProbeTimer,
						&Cancelled);
			  ...
			  IN UCHAR TxRate, IN BOOLEAN bQosNull)
{
	...
	TxWI =
	    (TXWI_STRUC *) & pAd->NullContext[0].TransferBuffer->
	    field.WirelessPacket[TXINFO_SIZE];
...
INT AsicReadAggCnt(RTMP_ADAPTER * pAd, ULONG * aggCnt, int cnt_len)
{
	...
	cnt = 0;
	for (i = 0; i < seg; i += 2) {
		for (reg_addr = aggReg[i]; reg_addr <= aggReg[i + 1];
		     reg_addr += 4) {
			...
			if (cnt < (cnt_len - 1)) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s():Get AggSize at Reg(0x%x) with val(0x%08x) [AGG_%d=>%ld, AGG_%d=>%ld]\n",
					  __FUNCTION__, reg_addr, reg_val.word,
					  cnt, aggCnt[cnt], cnt + 1,
					  aggCnt[cnt + 1]));
				cnt += 2;
			} else {
				DBGPRINT(RT_DEBUG_TRACE,
...
INT AsicSetChannel(RTMP_ADAPTER * pAd, UCHAR ch, UCHAR bw, UCHAR ext_ch,
		   BOOLEAN bScan)
{
...
VOID RT28xxAndesWOWEnable(IN PRTMP_ADAPTER pAd)
{
	...
	if (Ret != NDIS_STATUS_SUCCESS) {
		printk
		    ("\x1b[31m%s: send WOW config command failed(%d/%d)!!\x1b[m\n",
	...
	if (Ret != NDIS_STATUS_SUCCESS) {
		printk
		    ("\x1b[31m%s: send WOW config command failed!!(%d/%d)\x1b[m\n",
		     __FUNCTION__, CmdUnit.u.ANDES.Type, mask_cfg.Config_Type);
		return;
	}
	...
	if (pAd->StaCfg.AuthMode >= Ndis802_11AuthModeWPAPSK) {
		...
		NdisCopyMemory(sec_cfg.R_COUNTER, pEntry->R_Counter,
			       LEN_KEY_DESC_REPLAY);
	...
	if (Ret != NDIS_STATUS_SUCCESS) {
		printk
		    ("\x1b[31m%s: send WOW config command failed(%d/%d)!!\x1b[m\n",
		     __FUNCTION__, CmdUnit.u.ANDES.Type, infra_cfg.Config_Type);
		return;
	}
