...
BOOLEAN rtstrmactohex(PSTRING s1, PSTRING s2)
{
	...
	while ((*ptokS) != '\0') {
		if ((ptokE = strchr(ptokS, ':')) != NULL)
			*ptokE++ = '\0';
		if ((strlen(ptokS) != 2) || (!isxdigit(*ptokS))
		    || (!isxdigit(*(ptokS + 1))))
			break;	/* fail */
...
/*
    ...
    ========================================================================
*/
PSTRING RTMPFindSection(IN PSTRING buffer)
{
	STRING temp_buf[32];
	PSTRING ptr;
...
/*
    ...
    ========================================================================
*/
INT RTMPGetKeyParameterWithOffset(IN PSTRING key,
				  OUT PSTRING dest,
...
static int rtmp_parse_key_buffer_from_file(IN PRTMP_ADAPTER pAd,
					   IN PSTRING buffer, IN ULONG KeyType,
					   IN INT BSSIdx, IN INT KeyIdx)
{
	PSTRING keybuff;
...
static void rtmp_read_key_parms_from_file(IN PRTMP_ADAPTER pAd, PSTRING tmpbuf,
					  PSTRING buffer)
{
	STRING tok_str[16];
	PSTRING macptr;
					      ...
					      char *tmpbuf, char *buffer)
{
	...
	if (bWmmEnable) {
		/*APSDCapable */
		if (RTMPGetKeyParameter
		    ("APSDCapable", tmpbuf, 10, buffer, TRUE)) {
			if (simple_strtol(tmpbuf, 0, 10) != 0)	/*Enable */
		...
		/*MaxSPLength */
		if (RTMPGetKeyParameter
		    ("MaxSPLength", tmpbuf, 10, buffer, TRUE)) {
...
#ifdef XLINK_SUPPORT
static void rtmp_get_psp_xlink_mode_from_file(IN PRTMP_ADAPTER pAd,
					      char *tmpbuf, char *buffer)
{
	UINT32 Value = 0;
...
#ifdef DOT11_VHT_AC
static void VHTParametersHook(IN RTMP_ADAPTER * pAd,
			      IN PSTRING pValueStr, IN PSTRING pInput)
{
	long Value;
...
static void HTParametersHook(IN PRTMP_ADAPTER pAd,
			     IN PSTRING pValueStr, IN PSTRING pInput)
{
	long Value;
	...
	if (RTMPGetKeyParameter("HT_BW", pValueStr, 25, pInput, TRUE)) {
		...
		pAd->CommonCfg.MCastPhyMode.field.BW =
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			if ((Value >= 0 && Value <= 23) || (Value == 32)) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("HT: MCS = %d\n",
					  pAd->StaCfg.
					  DesiredTransmitSetting.field.MCS));
			} else {
				pAd->StaCfg.DesiredTransmitSetting.field.MCS =
				    MCS_AUTO;
	...
	/* 40_Mhz_Intolerant */
	if (RTMPGetKeyParameter
	    ("HT_40MHZ_INTOLERANT", pValueStr, 25, pInput, TRUE)) {
		Value = simple_strtol(pValueStr, 0, 10);
		if (Value == 0) {
	...
	if (RTMPGetKeyParameter("OBSSScanParam", pValueStr, 32, pInput, TRUE)) {
		...
		for (idx = 0, macptr = rstrtok(pValueStr, ";"); macptr;
			...
			case 5:
				pAd->
				    CommonCfg.Dot11BssWidthChanTranDelayFactor =
				    ObssScanValue;
				DBGPRINT(RT_DEBUG_TRACE,
	...
	if (RTMPGetKeyParameter
	    ("HT_BSSCoexistence", pValueStr, 25, pInput, TRUE)) {
		Value = simple_strtol(pValueStr, 0, 10);
	...
	if (RTMPGetKeyParameter
	    ("HT_BSSCoexApCntThr", pValueStr, 25, pInput, TRUE)) {
		pAd->CommonCfg.BssCoexApCntThr =
		    simple_strtol(pValueStr, 0, 10);;
...
void RTMPSetSTASSID(RTMP_ADAPTER * pAd, PSTRING SSID)
{
	...
	NdisMoveMemory(pAd->CommonCfg.LastSsid, SSID,
		       pAd->CommonCfg.LastSsidLen);
	pAd->MlmeAux.AutoReconnectSsidLen = pAd->CommonCfg.SsidLen;
	NdisZeroMemory(pAd->MlmeAux.AutoReconnectSsid, NDIS_802_11_LENGTH_SSID);
	NdisMoveMemory(pAd->MlmeAux.AutoReconnectSsid, SSID,
...
void RTMPSetSTAPassPhrase(RTMP_ADAPTER * pAd, PSTRING PassPh)
{
	...
	if ((pAd->StaCfg.AuthMode != Ndis802_11AuthModeWPAPSK) &&
		...
		ret =
		    RT_CfgSetWPAPSKKey(pAd, PassPh, strlen(PassPh),
	...
	if (ret == TRUE) {
		...
		RTMPMoveMemory(pAd->StaCfg.WpaPassPhrase, PassPh,
			       strlen(PassPh));
...
inline void RTMPSetSTACipherSuites(RTMP_ADAPTER * pAd,
...
NDIS_STATUS RecoverConnectInfo(IN RTMP_ADAPTER * pAd)
{
	...
	if ((pAd->StaCtIf.Changeable == FALSE)
	    || (pAd->StaCtIf.SsidLen > NDIS_802_11_LENGTH_SSID)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 (" DRIVER INIT  not need to RecoverConnectInfo() \n"));
	...
	for (idx = 0; idx < 4; idx++) {
		NdisMoveMemory(&pAd->SharedKey[BSS0][idx],
			       &pAd->StaCtIf.SharedKey[BSS0][idx],
			       sizeof(CIPHER_KEY));
#ifdef WPA_SUPPLICANT_SUPPORT
		NdisMoveMemory(&pAd->StaCfg.DesireSharedKey[idx],
			       &pAd->StaCtIf.SharedKey[BSS0][idx],
			       sizeof(CIPHER_KEY));
#endif // WPA_SUPPLICANT_SUPPORT //
...
NDIS_STATUS StoreConnectInfo(IN RTMP_ADAPTER * pAd)
{
	...
	NdisMoveMemory(pAd->StaCtIf.Ssid, pAd->CommonCfg.Ssid,
	...
	for (idx = 0; idx < 4; idx++) {
		NdisMoveMemory(&pAd->StaCtIf.SharedKey[BSS0][idx],
			       &pAd->SharedKey[BSS0][idx], sizeof(CIPHER_KEY));
	}
...
/*	tmpbuf = kmalloc(MAX_PARAM_BUFFER_SIZE, MEM_ALLOC_FLAG);*/
	...
	do {
		...
		if (RTMPGetKeyParameter
		    ("MacAddress", tmpbuf, 25, pBuffer, TRUE)) {
		...
		/*CountryRegion */
		if (RTMPGetKeyParameter
		    ("CountryRegion", tmpbuf, 25, pBuffer, TRUE)) {
		...
		/*CountryRegionABand */
		if (RTMPGetKeyParameter
		    ("CountryRegionABand", tmpbuf, 25, pBuffer, TRUE)) {
		...
		/*EfuseBufferMode */
		if (RTMPGetKeyParameter
		    ("EfuseBufferMode", tmpbuf, 25, pBuffer, TRUE)) {
		...
		if (pAd->CommonCfg.bCountryFlag == 0) {
			if (RTMPGetKeyParameter
			    ("CountryCode", tmpbuf, 25, pBuffer, TRUE))
				RTMPSetCountryCode(pAd, tmpbuf);
		}
#ifdef EXT_BUILD_CHANNEL_LIST
		/*ChannelGeography */
		if (RTMPGetKeyParameter
		    ("ChannelGeography", tmpbuf, 25, pBuffer, TRUE)) {
			UCHAR Geography = (UCHAR) simple_strtol(tmpbuf, 0, 10);
			if (Geography <= BOTH) {
				pAd->CommonCfg.Geography = Geography;
				pAd->CommonCfg.CountryCode[2] =
				    (pAd->CommonCfg.Geography ==
				     BOTH) ? ' ' : ((pAd->CommonCfg.Geography ==
						     IDOR) ? 'I' : 'O');
#ifdef CONFIG_STA_SUPPORT
#ifdef EXT_BUILD_CHANNEL_LIST
				IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
				    pAd->StaCfg.StaOriGeography =
				    pAd->CommonCfg.Geography;
#endif /* EXT_BUILD_CHANNEL_LIST */
#endif /* CONFIG_STA_SUPPORT */
				DBGPRINT(RT_DEBUG_TRACE,
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			/*NetworkType */
			if (RTMPGetKeyParameter
			    ("NetworkType", tmpbuf, 25, pBuffer, TRUE)) {
		...
		if (RTMPGetKeyParameter
		    ("WirelessMode", tmpbuf, 32, pBuffer, TRUE)) {
			UCHAR cfg_mode;
			for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
			     macptr = rstrtok(NULL, ";"), i++) {
		...
		if (RTMPGetKeyParameter("BasicRate", tmpbuf, 10, pBuffer, TRUE)) {
			pAd->CommonCfg.BasicRateBitmap =
			    (ULONG) simple_strtol(tmpbuf, 0, 10);
			pAd->CommonCfg.BasicRateBitmapOld =
			    (ULONG) simple_strtol(tmpbuf, 0, 10);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("BasicRate=%ld\n",
				  pAd->CommonCfg.BasicRateBitmap));
		...
		/*BeaconPeriod */
		if (RTMPGetKeyParameter
		    ("BeaconPeriod", tmpbuf, 10, pBuffer, TRUE)) {
		...
		/*BGProtection */
		if (RTMPGetKeyParameter
		    ("BGProtection", tmpbuf, 10, pBuffer, TRUE)) {
		...
		/*TxPreamble */
		if (RTMPGetKeyParameter
		    ("TxPreamble", tmpbuf, 10, pBuffer, TRUE)) {
			switch (simple_strtol(tmpbuf, 0, 10)) {
		...
		/*RTSThreshold */
		if (RTMPGetKeyParameter
		    ("RTSThreshold", tmpbuf, 10, pBuffer, TRUE)) {
		...
		/*FragThreshold */
		if (RTMPGetKeyParameter
		    ("FragThreshold", tmpbuf, 10, pBuffer, TRUE)) {
		...
		/*PktAggregate */
		if (RTMPGetKeyParameter
		    ("PktAggregate", tmpbuf, 10, pBuffer, TRUE)) {
			if (simple_strtol(tmpbuf, 0, 10) != 0)	/*Enable */
		...
		if (pAd->chipCap.FlgHwTxBfCap) {
			...
			/*ITxBfEn */
			if (RTMPGetKeyParameter
			    ("ITxBfEn", tmpbuf, 32, pBuffer, TRUE)) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("ITxBfEn = %d\n",
					  pAd->CommonCfg.
					  RegTransmitSetting.field.ITxBfEn));
			...
			/* ITxBfTimeout */
			if (RTMPGetKeyParameter
			    ("ITxBfTimeout", tmpbuf, 32, pBuffer, TRUE)) {
			...
			/* ETxBfEnCond */
			if (RTMPGetKeyParameter
			    ("ETxBfEnCond", tmpbuf, 32, pBuffer, TRUE)) {
			...
			/* ETxBfTimeout */
			if (RTMPGetKeyParameter
			    ("ETxBfTimeout", tmpbuf, 32, pBuffer, TRUE)) {
			...
			/* ETxBfNoncompress */
			if (RTMPGetKeyParameter
			    ("ETxBfNoncompress", tmpbuf, 32, pBuffer, TRUE)) {
			...
			/* ETxBfIncapable */
			if (RTMPGetKeyParameter
			    ("ETxBfIncapable", tmpbuf, 32, pBuffer, TRUE)) {
		...
		/*PreAntSwitch */
		if (RTMPGetKeyParameter
		    ("PreAntSwitch", tmpbuf, 32, pBuffer, TRUE)) {
		...
		if (pAd->chipCap.FlgHwStreamMode) {
			if (RTMPGetKeyParameter
			    ("StreamMode", tmpbuf, 32, pBuffer, TRUE)) {
				pAd->CommonCfg.StreamMode =
				    (simple_strtol(tmpbuf, 0, 10) & 0x03);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("StreamMode= %d\n",
					  pAd->CommonCfg.StreamMode));
			}
			...
			for (i = 0; i < STREAM_MODE_STA_NUM; i++) {
				...
				if (RTMPGetKeyParameter
				    (tok_str, tmpbuf, MAX_PARAM_BUFFER_SIZE,
				     pBuffer, TRUE)) {
					int j;
					if (strlen(tmpbuf) != 17)	/*Mac address acceptable format 01:02:03:04:05:06 length 17 */
						continue;

					for (j = 0; j < ETH_LENGTH_OF_ADDRESS;
					     j++) {
						AtoH(tmpbuf,
						     &pAd->
						     CommonCfg.StreamModeMac[i]
						     [j], 1);
						tmpbuf = tmpbuf + 3;
					}
				}
			...
			if (NdisEqualMemory
			    (ZERO_MAC_ADDR, &pAd->CommonCfg.StreamModeMac[0][0],
			     MAC_ADDR_LEN)) {
				/* set default broadcast mac to entry 0 if user not set it */
				NdisMoveMemory(&pAd->
					       CommonCfg.StreamModeMac[0][0],
		...
		/*DebugFlags */
		if (RTMPGetKeyParameter
		    ("DebugFlags", tmpbuf, 32, pBuffer, TRUE)) {
			pAd->CommonCfg.DebugFlags =
			    simple_strtol(tmpbuf, 0, 16);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("DebugFlags = 0x%02lx\n",
				  pAd->CommonCfg.DebugFlags));
		}
		...
		/*IEEE80211H */
		if (RTMPGetKeyParameter
		    ("IEEE80211H", tmpbuf, 10, pBuffer, TRUE)) {
			for (i = 0, macptr = rstrtok(tmpbuf, ";"); macptr;
			     macptr = rstrtok(NULL, ";"), i++) {
...
#ifdef DFS_SUPPORT
		{
			/*CSPeriod */
			if (RTMPGetKeyParameter
			    ("CSPeriod", tmpbuf, 10, pBuffer, TRUE)) {
		...
		/*WirelessEvent */
		if (RTMPGetKeyParameter
		    ("WirelessEvent", tmpbuf, 10, pBuffer, TRUE)) {
			BOOLEAN FlgIsWEntSup = FALSE;

			if (simple_strtol(tmpbuf, 0, 10) != 0)
				FlgIsWEntSup = TRUE;

			RtmpOsWlanEventSet(pAd, &pAd->CommonCfg.bWirelessEvent,
					   FlgIsWEntSup);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("WirelessEvent=%d\n",
...
#ifdef CONFIG_STA_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
				if ((strcmp(tmpbuf, "WEPAUTO") == 0)
				    || (strcmp(tmpbuf, "wepauto") == 0))
					pAd->StaCfg.AuthMode =
					    Ndis802_11AuthModeAutoSwitch;
				else if ((strcmp(tmpbuf, "SHARED") == 0)
					 || (strcmp(tmpbuf, "shared") == 0))
					pAd->StaCfg.AuthMode =
					    Ndis802_11AuthModeShared;
				else if ((strcmp(tmpbuf, "WPAPSK") == 0)
					 || (strcmp(tmpbuf, "wpapsk") == 0))
					pAd->StaCfg.AuthMode =
					    Ndis802_11AuthModeWPAPSK;
				else if ((strcmp(tmpbuf, "WPANONE") == 0)
					 || (strcmp(tmpbuf, "wpanone") == 0))
					pAd->StaCfg.AuthMode =
					    Ndis802_11AuthModeWPANone;
				else if ((strcmp(tmpbuf, "WPA2PSK") == 0)
					 || (strcmp(tmpbuf, "wpa2psk") == 0))
					pAd->StaCfg.AuthMode =
					    Ndis802_11AuthModeWPA2PSK;
#ifdef WPA_SUPPLICANT_SUPPORT
				else if ((strcmp(tmpbuf, "WPA") == 0)
					 || (strcmp(tmpbuf, "wpa") == 0))
					pAd->StaCfg.AuthMode =
					    Ndis802_11AuthModeWPA;
				else if ((strcmp(tmpbuf, "WPA2") == 0)
					 || (strcmp(tmpbuf, "wpa2") == 0))
					pAd->StaCfg.AuthMode =
		...
		/*EncrypType */
		if (RTMPGetKeyParameter
		    ("EncrypType", tmpbuf, 128, pBuffer, TRUE)) {

#ifdef CONFIG_STA_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
				if ((strcmp(tmpbuf, "WEP") == 0)
				    || (strcmp(tmpbuf, "wep") == 0))
					pAd->StaCfg.WepStatus =
					    Ndis802_11WEPEnabled;
				else if ((strcmp(tmpbuf, "TKIP") == 0)
					 || (strcmp(tmpbuf, "tkip") == 0))
					pAd->StaCfg.WepStatus =
					    Ndis802_11Encryption2Enabled;
				else if ((strcmp(tmpbuf, "AES") == 0)
					 || (strcmp(tmpbuf, "aes") == 0))
					pAd->StaCfg.WepStatus =
		...
		/*CarrierDetect */
		if (RTMPGetKeyParameter
		    ("CarrierDetect", tmpbuf, 128, pBuffer, TRUE)) {
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			if (RTMPGetKeyParameter
				...
				if (pAd->StaCfg.BssType == BSS_INFRA) {
					if ((strcmp(tmpbuf, "MAX_PSP") == 0)
						...
						if (pAd->
						    StaCfg.bWindowsACCAMEnable
						    == FALSE)
							pAd->
							    StaCfg.WindowsPowerMode
							    =
							    Ndis802_11PowerModeMAX_PSP;
						pAd->
						    StaCfg.WindowsBatteryPowerMode
						    =
						    Ndis802_11PowerModeMAX_PSP;
						pAd->StaCfg.DefaultListenCount =
						    5;
					} else
					    if ((strcmp(tmpbuf, "Fast_PSP") ==
						 0)
						|| (strcmp(tmpbuf, "fast_psp")
						    == 0)
						|| (strcmp(tmpbuf, "FAST_PSP")
						    == 0)) {
						/* do NOT turn on PSM bit here, wait until MlmeCheckForPsmChange() */
					    ...
					    if ((strcmp(tmpbuf, "Legacy_PSP") ==
						 0)
						|| (strcmp(tmpbuf, "legacy_psp")
						    == 0)
						|| (strcmp(tmpbuf, "LEGACY_PSP")
						    == 0)) {
						/* do NOT turn on PSM bit here, wait until MlmeCheckForPsmChange() */
			...
			/* AutoRoaming by RSSI */
			if (RTMPGetKeyParameter
			    ("AutoRoaming", tmpbuf, 32, pBuffer, TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) == 0)
			...
			/* RoamThreshold */
			if (RTMPGetKeyParameter
			    ("RoamThreshold", tmpbuf, 32, pBuffer, TRUE)) {
			...
			if (RTMPGetKeyParameter
			    ("TGnWifiTest", tmpbuf, 10, pBuffer, TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) == 0)
			...
			/* Beacon Lost Time */
			if (RTMPGetKeyParameter
			    ("BeaconLostTime", tmpbuf, 32, pBuffer, TRUE)) {
				ULONG lInfo =
				    (ULONG) simple_strtol(tmpbuf, 0, 10);
			...
			if (RTMPGetKeyParameter
			    ("AutoConnect", tmpbuf, 32, pBuffer, TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) == 0)
					pAd->StaCfg.bAutoConnectIfNoSSID =
					    FALSE;
				else
			...
			/* FastConnect */
			if (RTMPGetKeyParameter
			    ("FastConnect", tmpbuf, 32, pBuffer, TRUE)) {
				if (simple_strtol(tmpbuf, 0, 10) == 0)
		...
		if (RTMPGetKeyParameter
		    ("BandedgeDelta", tmpbuf, 10, pBuffer, TRUE)) {
		...
		if (RTMPGetKeyParameter
		    ("WOW_Enable", tmpbuf, 10, pBuffer, TRUE))
		...
		if (RTMPGetKeyParameter
		    ("WOW_InBand", tmpbuf, 10, pBuffer, TRUE))
			Set_WOW_InBand(pAd, tmpbuf);
#endif /* (defined(WOW_SUPPORT) && defined(RTMP_MAC_USB)) || defined(NEW_WOW_SUPPORT) */

#ifdef MICROWAVE_OVEN_SUPPORT
		if (RTMPGetKeyParameter
		    ("MO_FalseCCATh", tmpbuf, 10, pBuffer, TRUE)) {
...
#define LETTER_CASE_TRANSLATE(txt_p, card_id)			\
	...
	if (retval < 0) {
		...
		if ((card_same_mac_id >= 0) &&
			...
			if (card_select_method == MC_SELECT_CARDTYPE) {
				...
				snprintf(card_id_buf, sizeof(card_id_buf),
					 "%02dCARDTYPE%s", card_match_id,
					 RFIC_word);

				if ((start_ptr =
				     (PUCHAR) rtstrstruncasecmp(buffer,
								card_id_buf)) !=
				    NULL) {
			...
			case MC_SELECT_CARDTYPE:	/* CARDTYPE */
				...
				for (card_index = 0;
				     ...
				     card_index++) {
					snprintf(card_id_buf,
						 sizeof(card_id_buf),
						 "%02dCARDTYPE%s", card_index,
						 RFIC_word);
		...
		if (card_match_id >= 0) {
			...
			/* read card file path */
			if (RTMPGetKeyParameter
			    (card_id_buf, tmpbuf, 256, buffer, TRUE)) {
				if (strlen(tmpbuf) < sizeof(pAd->MC_FileName)) {
					/* backup card information */
					pAd->MC_RowID = card_match_id;	/* base 0 */
					MC_CardUsed[card_match_id] = 1;
					memcpy(MC_CardMac[card_match_id], mac,
					       sizeof(mac));

					/* backup card file path */
					NdisMoveMemory(pAd->MC_FileName, tmpbuf,
						       strlen(tmpbuf));
			...
			if ((flg_match_ok != TRUE) &&
				...
				memset(MC_CardMac[card_match_id], 0,
				       sizeof(mac));
			}
		}		/* if (card_match_id >= 0) */
	}
				 ...
				 IN PSTRING tmpbuf, IN PSTRING buffer)
{
	...
	/*WdsPhyMode */
	if (RTMPGetKeyParameter
	    ("WdsPhyMode", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
		     (macptr && i < MAX_WDS_ENTRY);
		     macptr = rstrtok(NULL, ";"), i++) {
			if ((strncmp(macptr, "CCK", 3) == 0)
			    || (strncmp(macptr, "cck", 3) == 0))
				pAd->MulTestTab.WdsEntry[i].PhyMode = MODE_CCK;
			else if ((strncmp(macptr, "OFDM", 4) == 0)
				 || (strncmp(macptr, "ofdm", 4) == 0))
				pAd->MulTestTab.WdsEntry[i].PhyMode = MODE_OFDM;
#ifdef DOT11_N_SUPPORT
			else if ((strncmp(macptr, "HTMIX", 5) == 0)
				 || (strncmp(macptr, "htmix", 5) == 0))
				pAd->MulTestTab.WdsEntry[i].PhyMode =
				    MODE_HTMIX;
			else if ((strncmp(macptr, "GREENFIELD", 10) == 0)
				 || (strncmp(macptr, "greenfield", 10) == 0))
				pAd->MulTestTab.WdsEntry[i].PhyMode =
				    MODE_HTGREENFIELD;
#endif /* DOT11_N_SUPPORT */
			else
	...
	/*WdsList */
	if (RTMPGetKeyParameter
	    ("WdsList", tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
		     (macptr && i < MAX_WDS_ENTRY);
		     macptr = rstrtok(NULL, ";"), i++) {
...

			{
				...
				for (iii = 0; iii < MAX_WDS_ENTRY; iii++) {
					if (pAd->MulTestTab.
					    WdsEntry[iii].Valid == FALSE) {
						pAd->MulTestTab.
						    WdsEntry[iii].Valid = TRUE;
						pAd->MulTestTab.Size++;
						COPY_MAC_ADDR(pAd->
							      MulTestTab.WdsEntry
							      [iii].PeerWdsAddr,
				...
				if (iii == MAX_WDS_ENTRY)
					DBGPRINT(RT_DEBUG_ERROR,
						 ("%s: Unable to allocate MulTestEntry.\n",
						  __FUNCTION__));
			}
		}
	}
	...
	if (RTMPGetKeyParameter("WdsEncrypType", tmpbuf, 128, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			if ((strncmp(macptr, "NONE", 4) == 0)
				...
				pAd->MulTestTab.WdsEntry[i].WepStatus =
				    Ndis802_11WEPDisabled;
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("WdsEncrypType[%d]=%d(%s)\n", i,
				  pAd->MulTestTab.WdsEntry[i].WepStatus,
				  GetEncryptType(pAd->MulTestTab.
						 WdsEntry[i].WepStatus)));
		}
		...
		if (i == 1) {
			for (j = 1; j < MAX_WDS_ENTRY; j++) {
				pAd->MulTestTab.WdsEntry[j].WepStatus =
				    pAd->MulTestTab.WdsEntry[0].WepStatus;
				DBGPRINT(RT_DEBUG_TRACE,
					 ("@WdsEncrypType[%d]=%d(%s)\n", j,
					  pAd->MulTestTab.WdsEntry[i].WepStatus,
					  GetEncryptType(pAd->
	...
	if (RTMPGetKeyParameter("WdsKey", tmpbuf, 255, buffer, FALSE)) {
		for (i = 0; i < MAX_WDS_ENTRY; i++)
			NdisZeroMemory(&pAd->MulTestTab.WdsEntry[i].WdsKey,
				       sizeof(CIPHER_KEY));
		...
		if (((pAd->MulTestTab.WdsEntry[0].WepStatus ==
		      Ndis802_11Encryption2Enabled)
		     || (pAd->MulTestTab.WdsEntry[0].WepStatus ==
			 Ndis802_11Encryption3Enabled))
		    && (strlen(tmpbuf) >= 8) && (strlen(tmpbuf) <= 64)) {
			RT_CfgSetWPAPSKKey(pAd, tmpbuf, strlen(tmpbuf),
					   (PUCHAR) RALINK_PASSPHRASE,
					   sizeof(RALINK_PASSPHRASE),
					   keyMaterial);
			if (pAd->MulTestTab.WdsEntry[0].WepStatus ==
			    Ndis802_11Encryption3Enabled)
				pAd->MulTestTab.WdsEntry[0].WdsKey.CipherAlg =
				    CIPHER_AES;
			else
				pAd->MulTestTab.WdsEntry[0].WdsKey.CipherAlg =
				    CIPHER_TKIP;

			NdisMoveMemory(&pAd->MulTestTab.WdsEntry[0].WdsKey.Key,
				       keyMaterial, 16);
			pAd->MulTestTab.WdsEntry[0].WdsKey.KeyLen = 16;
			NdisMoveMemory(&pAd->MulTestTab.WdsEntry[0].
				       WdsKey.RxMic, keyMaterial + 16, 8);
			NdisMoveMemory(&pAd->MulTestTab.WdsEntry[0].
				       WdsKey.TxMic, keyMaterial + 16, 8);
		}
	...
	if (!bUsePrevFormat) {
		for (i = 0; i < MAX_WDS_ENTRY; i++) {
			...
			if (RTMPGetKeyParameter
			    (tok_str, tmpbuf, 128, buffer, FALSE)) {
				if (pAd->MulTestTab.WdsEntry[i].WepStatus ==
					...
					if (strlen(tmpbuf) == 5
						...
						pAd->MulTestTab.
						    ...
						    CipherAlg = CipherAlg;
						DBGPRINT(RT_DEBUG_TRACE,
							 ("IF/wds%d Key=%s ,type=Ascii, CipherAlg(%s)\n",
							  i, tmpbuf,
							  (CipherAlg ==
							   CIPHER_WEP64 ?
							   "wep64" :
					...
					/* Hex type */
						...
						pAd->MulTestTab.
						    ...
						    CipherAlg = CipherAlg;
						DBGPRINT(RT_DEBUG_TRACE,
							 ("IF/wds%d Key=%s ,type=Hex, CipherAlg(%s)\n",
							  i, tmpbuf,
							  (CipherAlg ==
							   CIPHER_WEP64 ?
							   "wep64" :
					...
					/* Invalid type */
						...
						NdisZeroMemory(&pAd->
							       ...
							       (CIPHER_KEY));
						DBGPRINT(RT_DEBUG_TRACE,
							 ("IF/wds%d has invalid key for WEP, reset encryption to OPEN\n",
							  i));
					}
				} else
				    if ((pAd->MulTestTab.
					...
					if ((strlen(tmpbuf) >= 8)
						...
						DBGPRINT(RT_DEBUG_TRACE,
							 ("IF/wds%d has invalid key for WPA, reset encryption to OPEN\n",
							  i));
						pAd->MulTestTab.
						    WdsEntry[i].WepStatus =
						    Ndis802_11EncryptionDisabled;
	...
	if (RTMPGetKeyParameter("WdsDefaultKeyID", tmpbuf, 10, buffer, TRUE)) {
		for (i = 0, macptr = rstrtok(tmpbuf, ";");
			...
			if ((KeyIdx >= 1) && (KeyIdx <= 4))
				pAd->MulTestTab.WdsEntry[i].KeyIdx =
				    (UCHAR) (KeyIdx - 1);
			else
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IF/wds%d - WdsDefaultKeyID(0~3)=%d\n", i,
				  pAd->MulTestTab.WdsEntry[i].KeyIdx));
		}
	}
}
...
NDIS_STATUS RTMPSetSingleSKUParameters(IN RTMP_ADAPTER * pAd)
{
		...
		for (readline = ptr = buffer, index = 0;
			...
			*ptr = '\0';
			...
			if (!strncmp(readline, "ch", 2)) {
				...
				for (i = 0; i < SINGLE_SKU_TABLE_CCK_LENGTH;
				     i++) {
					token = rstrtok(NULL, " ");
					if (token == NULL)
						break;
				...
				for (i = 0; i < SINGLE_SKU_TABLE_OFDM_LENGTH;
				     i++) {
					token = rstrtok(NULL, " ");
					if (token == NULL)
						break;
				...
				if (StartCh == NULL) {
					...
					if (isSame == TRUE) {
						for (i = 0;
						     i <
						     SINGLE_SKU_TABLE_OFDM_LENGTH;
						     i++) {
							if (StartCh->PwrOFDM[i]
							    !=
							    pwr->PwrOFDM[i]) {
								isSame = FALSE;
								break;
					...
					if (isSame == TRUE) {
						for (i = 0;
						     i <
						     SINGLE_SKU_TABLE_HT_LENGTH;
						     i++) {
							if (StartCh->PwrHT20[i]
							    !=
							    pwr->PwrHT20[i]) {
								isSame = FALSE;
								break;
	...
	{
		CH_POWER *ch, *ch_temp;
		DlListForEachSafe(ch, ch_temp, &pAd->SingleSkuPwrList, CH_POWER,
				  List) {
			int i;
			DBGPRINT(RT_DEBUG_TRACE,
				 ("start ch = %d, ch->num = %d\n",
		...
		if (channel >= start_ch) {
			for (j = 0; j < ch->num; j++) {
				if (channel == ch->Channel[j]) {
					...
					for (i = 0;
					     i < SINGLE_SKU_TABLE_OFDM_LENGTH;
					     i++) {
...
VOID InitSkuRateDiffTable(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < 9; i++) {
		RT28xx_EEPROM_READ16(pAd,
				     EEPROM_TXPOWER_BYRATE_20MHZ_2_4G + i * 2,
				     value);
		Pwr = value & 0xFF;
...
INT32 GetSkuPAModePwr(IN PRTMP_ADAPTER pAd, IN UCHAR PAMode)
{
	INT32 pa_mode_pwr = 0;
		    ...
		    IN CHAR phymode, IN UCHAR channel, IN UCHAR bw)
{
	...
	pwr_diff =
	    GetSkuPAModePwr(pAd,
			    phymode) +
	    (pAd->SingleSkuRatePwrDiff[phymode] << 12) + 2048;
		      ...
		      IN UCHAR channel, IN UCHAR bw, IN CHAR base_pwr)
{
	...
	for (i = 0; i < 5; i++) {
		...
		data =
		    (rate_table[i * 4] & 0x3F) +
		    ((rate_table[i * 4 + 1] & 0x3F) << 8);
		Adata =
		    ((rate_table[i * 4] + pAd->chipCap.Apwrdelta) & 0x3F) +
		    (((rate_table[i * 4 + 1] +
		       pAd->chipCap.Apwrdelta) & 0x3F) << 8);
		Gdata =
		    ((rate_table[i * 4] + pAd->chipCap.Gpwrdelta) & 0x3F) +
		    (((rate_table[i * 4 + 1] +
		       pAd->chipCap.Gpwrdelta) & 0x3F) << 8);

		if (i != 4) {
			data |=
			    ((rate_table[i * 4 + 2] & 0x3F) << 16) +
			    ((rate_table[i * 4 + 3] & 0x3F) << 24);
			Adata |=
			    (((rate_table[i * 4 + 2] +
			       pAd->chipCap.Apwrdelta) & 0x3F) << 16) +
			    (((rate_table[i * 4 + 3] +
			       pAd->chipCap.Apwrdelta) & 0x3F) << 24);
			Gdata |=
			    (((rate_table[i * 4 + 2] +
			       pAd->chipCap.Gpwrdelta) & 0x3F) << 16) +
			    (((rate_table[i * 4 + 3] +
			       pAd->chipCap.Gpwrdelta) & 0x3F) << 24);
		}
		...
		DBGPRINT_RAW(RT_DEBUG_TRACE,
			     ("20MHz BW, 2.4G band-%08x,  Adata = %08x,  Gdata = %08x \n",
			      data, Adata, Gdata));
	}
