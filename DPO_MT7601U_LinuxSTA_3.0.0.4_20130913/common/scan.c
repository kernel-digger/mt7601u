...
static INT scan_active(RTMP_ADAPTER * pAd, UCHAR OpMode, UCHAR ScanType)
{
...

	{
		...
		if (pAd->CommonCfg.ExtRateLen) {
			...
			MakeOutgoingFrame(frm_buf + FrameLen, &Tmp,
					  ...
					  pAd->CommonCfg.ExtRate, END_OF_ARGS);
			FrameLen += Tmp;
		}
	}
#ifdef DOT11_N_SUPPORT
	...
	if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) && (pAd->MlmeAux.Channel > 14)) {
		FrameLen +=
		    build_vht_ies(pAd, (UCHAR *) (frm_buf + FrameLen),
				  SUBTYPE_PROBE_REQ);
	}
#endif /* DOT11_VHT_AC */
	...
	if ((OpMode == OPMODE_STA) &&
		...
		MakeOutgoingFrame(frm_buf + FrameLen, &WpsTmpLen,
				  ...
				  pAd->StaCfg.pWpsProbeReqIe, END_OF_ARGS);
...
VOID ScanNextChannel(IN PRTMP_ADAPTER pAd, IN UCHAR OpMode)
{
	...
	ScanPending = ((pAd->StaCfg.bImprovedScan)
		       && (pAd->StaCfg.ScanChannelCnt >=
			   ImprovedScan_MaxScanChannelCnt));
#endif /* CONFIG_STA_SUPPORT */
	...
	if ((pAd->MlmeAux.Channel == 0) || ScanPending) {
		...
		if (CHAN_PropertyCheck
		    (pAd, pAd->MlmeAux.Channel, CHANNEL_PASSIVE_SCAN) == TRUE)
			ScanType = SCAN_PASSIVE;
