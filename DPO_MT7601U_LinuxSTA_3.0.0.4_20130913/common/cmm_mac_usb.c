...
static NDIS_STATUS RTMPFreeUsbBulkBufStruct(IN RTMP_ADAPTER * pAd,
					    IN PURB * ppUrb,
					    IN PUCHAR * ppXBuffer,
					    IN INT bufLen,
					    IN ra_dma_addr_t data_dma)
{
	...
	if (NULL != *ppXBuffer) {
		RTUSB_URB_FREE_BUFFER(pObj->pUsb_Dev, bufLen, *ppXBuffer,
				      data_dma);
		*ppXBuffer = NULL;
	}
...
/*	COUNTER_802_11	pCounter = &pAd->WlanCounters;*/
	...
	do {
		...
		for (num = 0; num < NUM_OF_TX_RING; num++) {
			NdisAllocateSpinLock(pAd,
					     &pAd->TxContextQueueLock[num]);
		}
...
VOID RTMPFreeTxRxRingMemory(IN PRTMP_ADAPTER pAd)
{
	...
	if (pCmdRspEventContext) {
		RTMPFreeUsbBulkBufStruct(pAd,
					 &pCmdRspEventContext->pUrb,
					 (PUCHAR *) &
					 pCmdRspEventContext->TransferBuffer,
					 CMD_RSP_BULK_SIZE,
					 pCmdRspEventContext->data_dma);
	}
...
VOID RTUSBBssBeaconInit(IN RTMP_ADAPTER * pAd)
{
	...
	if (pAd->CommonCfg.pBeaconSync) {
		...
		for (i = 0; i < HW_BEACON_MAX_COUNT(pAd); i++) {
			...
			pBeaconSync->CapabilityInfoLocationInBeacon[i] = 0;
			pBeaconSync->TimIELocationInBeacon[i] = 0;
			os_alloc_mem(pAd, &pBeaconSync->BeaconTxWI[i],
				     TXWISize);
			if (pBeaconSync->BeaconTxWI[i])
				NdisZeroMemory(pBeaconSync->BeaconTxWI[i],
					       TXWISize);
			else
				goto error2;
		}
...
VOID RT28xxUsbMlmeRadioOFF(IN PRTMP_ADAPTER pAd)
{
	...
	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)) {
		if (INFRA_ON(pAd) || ADHOC_ON(pAd)) {
			...
			os_alloc_mem(pAd, (UCHAR **) & pMsgElem,
...
VOID RT28xxUsbAsicWOWEnable(IN PRTMP_ADAPTER pAd)
{
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("Send WOW enable cmd (%d/%d/%d)\n", pAd->WOW_Cfg.nDelay,
		  pAd->WOW_Cfg.nSelectedGPIO, pAd->WOW_Cfg.nHoldTime));
	RTMP_IO_READ32(pAd, GPIO_HOLDTIME_OFFSET, &Value);
