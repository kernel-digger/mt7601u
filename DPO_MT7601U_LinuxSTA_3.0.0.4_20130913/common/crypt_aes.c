...
========================================================================
*/
VOID RT_AES_KeyExpansion(IN UINT8 Key[],
			 IN UINT KeyLength, INOUT AES_CTX_STRUC * paes_ctx)
{
	UINT KeyIndex = 0;
	...
	NumberOfWordOfKeyExpansion =
	    ((UINT) AES_KEY_ROWS) * ((KeyLength >> 2) + 6 + 1);
	while (KeyIndex < NumberOfWordOfKeyExpansion) {
		...
		if ((KeyIndex % NumberOfWordOfKey) == 0) {
			...
			Temp =
			    aes_sbox_enc[TempWord[1]] ^ ((Temprcon >> 24) &
							 0xff);
			TempWord[1] =
			    aes_sbox_enc[TempWord[2]] ^ ((Temprcon >> 16) &
							 0xff);
			TempWord[2] =
			    aes_sbox_enc[TempWord[3]] ^ ((Temprcon >> 8) &
							 0xff);
			TempWord[3] =
			    aes_sbox_enc[TempWord[0]] ^ ((Temprcon) & 0xff);
			TempWord[0] = Temp;
...
VOID RT_AES_Encrypt(IN UINT8 PlainBlock[],
		    ...
		    IN UINT8 Key[],
		    IN UINT KeyLength,
		    OUT UINT8 CipherBlock[], INOUT UINT * CipherBlockSize)
{
	...
	if ((KeyLength != AES_KEY128_LENGTH) && (KeyLength != AES_KEY192_LENGTH)
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("RT_AES_Encrypt: key length is %d bytes, it must be %d, %d, or %d bytes(128, 192, or 256 bits).\n",
			  KeyLength, AES_KEY128_LENGTH, AES_KEY192_LENGTH,
			  AES_KEY256_LENGTH));
		return;
	}
	...
	for (RoundIndex = 1; RoundIndex < NumberOfRound; RoundIndex++) {
		...
		for (RowIndex = 0; RowIndex < AES_STATE_ROWS; RowIndex++)
			for (ColumnIndex = 0; ColumnIndex < AES_STATE_COLUMNS;
			     ColumnIndex++)
				paes_ctx->State[RowIndex][ColumnIndex] =
				    aes_sbox_enc[paes_ctx->State[RowIndex]
						 [ColumnIndex]];
...
VOID RT_AES_Decrypt(IN UINT8 CipherBlock[],
		    ...
		    IN UINT8 Key[],
		    IN UINT KeyLength,
		    OUT UINT8 PlainBlock[], INOUT UINT * PlainBlockSize)
{
/*    AES_CTX_STRUC aes_ctx;
*/
	AES_CTX_STRUC *paes_ctx = NULL;
	...
	for (RoundIndex = (NumberOfRound - 1); RoundIndex > 0; RoundIndex--) {
		...
		for (RowIndex = 0; RowIndex < AES_STATE_ROWS; RowIndex++)
			for (ColumnIndex = 0; ColumnIndex < AES_STATE_COLUMNS;
			     ColumnIndex++)
				paes_ctx->State[RowIndex][ColumnIndex] =
				    aes_sbox_dec[paes_ctx->State[RowIndex]
						 [ColumnIndex]];
		...
		for (ColumnIndex = 0; ColumnIndex < AES_STATE_COLUMNS;
			...
			Row3 = paes_ctx->State[3][ColumnIndex];
			paes_ctx->State[0][ColumnIndex] =
			    aes_mul_e[Row0] ^ aes_mul_b[Row1] ^ aes_mul_d[Row2]
			    ^ aes_mul_9[Row3];
			paes_ctx->State[1][ColumnIndex] =
			    aes_mul_9[Row0] ^ aes_mul_e[Row1] ^ aes_mul_b[Row2]
			    ^ aes_mul_d[Row3];
			paes_ctx->State[2][ColumnIndex] =
			    aes_mul_d[Row0] ^ aes_mul_9[Row1] ^ aes_mul_e[Row2]
			    ^ aes_mul_b[Row3];
			paes_ctx->State[3][ColumnIndex] =
			    aes_mul_b[Row0] ^ aes_mul_d[Row1] ^ aes_mul_9[Row2]
			    ^ aes_mul_e[Row3];
		}
	}
...
VOID AES_CCM_MAC(IN UINT8 Payload[],
		 ...
		 IN UINT NonceLength,
		 IN UINT8 AAD[],
		 IN UINT AADLength, IN UINT MACLength, OUT UINT8 MACText[])
{
	...
	RT_AES_Encrypt(Block, AES_BLOCK_SIZES, Key, KeyLength, Block_MAC,
		       &Temp_Length);
	...
	while (ADD_Index < AADLength) {
		...
		for (Temp_Index = 0; Temp_Index < Copy_Length; Temp_Index++)
			Block[Temp_Index + Block_Index] =
			    AAD[ADD_Index + Temp_Index];
...
INT AES_CCM_Encrypt(IN UINT8 PlainText[],
		    ...
		    IN UINT AADLength,
		    IN UINT MACLength,
		    OUT UINT8 CipherText[], INOUT UINT * CipherTextLength)
{
	...
	if ((MACLength != 4) && (MACLength != 6) && (MACLength != 8)
	    && (MACLength != 10)
	 ...
	 */
	AES_CCM_MAC(PlainText, PlainTextLength, Key, KeyLength, Nonce,
		    NonceLength, AAD, AADLength, MACLength, Block_MAC);
	Temp_Length = sizeof(Block_CTR_Cipher);
	RT_AES_Encrypt(Block_CTR, AES_BLOCK_SIZES, Key, KeyLength,
		       Block_CTR_Cipher, &Temp_Length);
	for (Temp_Index = 0; Temp_Index < MACLength; Temp_Index++)
	...
	while (Cipher_Index < PlainTextLength) {
		...
		for (Temp_Index = 0; Temp_Index < Copy_Length; Temp_Index++)
			CipherText[Cipher_Index + Temp_Index] =
			    PlainText[Cipher_Index +
				      Temp_Index] ^
			    Block_CTR_Cipher[Temp_Index];

		Cipher_Index += Copy_Length;
	}
	for (Temp_Index = 0; Temp_Index < MACLength; Temp_Index++)
		CipherText[PlainTextLength + Temp_Index] =
...
INT AES_CCM_Decrypt(IN UINT8 CipherText[],
		    ...
		    IN UINT AADLength,
		    IN UINT MACLength,
		    OUT UINT8 PlainText[], INOUT UINT * PlainTextLength)
{
	UINT8 Block_MAC[AES_BLOCK_SIZES],
	    Block_MAC_From_Cipher[AES_BLOCK_SIZES];
	UINT8 Block_CTR[AES_BLOCK_SIZES], Block_CTR_Cipher[AES_BLOCK_SIZES];
	 ...
	 */
	Block_Index = 0;
	for (Temp_Index = (CipherTextLength - MACLength);
	     Temp_Index < CipherTextLength; Temp_Index++, Block_Index++)
...
VOID AES_CMAC_GenerateSubKey(IN UINT8 Key[],
			     IN UINT KeyLength,
			     OUT UINT8 SubKey1[], OUT UINT8 SubKey2[])
{
	...
	/* Step 1: L := AES-128(K, const_Zero); */
	SubKey1_Length = 16;
	RT_AES_Encrypt(Const_Zero, sizeof(Const_Zero), Key, KeyLength, SubKey1,
		       &SubKey1_Length);
...
VOID AES_CMAC(IN UINT8 PlainText[],
	      ...
	      IN UINT8 Key[],
	      IN UINT KeyLength,
	      OUT UINT8 MACText[], INOUT UINT * MACTextLength)
{
	...
	while ((PlainTextLength - PlainBlockStart) > AES_BLOCK_SIZES) {
		for (Index = 0; Index < AES_BLOCK_SIZES; Index++)
			Y[Index] =
			    PlainText[PlainBlockStart + Index] ^ X[Index];
		...
		for (Index = 0; Index < AES_BLOCK_SIZES; Index++)
			Y[Index] =
			    PlainText[PlainBlockStart +
				      Index] ^ X[Index] ^ SubKey1[Index];
	} else {
		NdisZeroMemory(Y, AES_BLOCK_SIZES);
		NdisMoveMemory(Y, &PlainText[PlainBlockStart],
		     ...
		     OUT UINT8 CipherText[], INOUT UINT * CipherTextLength)
{
	 ...
	 */
	PaddingSize =
	    ((UINT) AES_BLOCK_SIZES) -
	    (PlainTextLength % ((UINT) AES_BLOCK_SIZES));
	...
	NdisMoveMemory(Block, (&PlainText[0] + PlainBlockStart),
		       (PlainTextLength - PlainBlockStart));
	NdisFillMemory((Block + (((UINT) AES_BLOCK_SIZES) - PaddingSize)),
		     ...
		     OUT UINT8 PlainText[], INOUT UINT * PlainTextLength)
{
	...
	while ((CipherTextLength - CipherBlockStart) >= AES_BLOCK_SIZES) {
		...
		RT_AES_Decrypt(CipherText + CipherBlockStart, AES_BLOCK_SIZES,
			       Key, KeyLength, PlainText + PlainBlockStart,
			       &PlainBlockSize);
		...
		if (PlainBlockStart == 0) {
			...
			for (Index = 0; Index < AES_BLOCK_SIZES; Index++)
				PlainText[PlainBlockStart + Index] ^=
				    CipherText[CipherBlockStart + Index -
					       ((UINT) AES_BLOCK_SIZES)];
		}
...
INT AES_Key_Wrap(IN UINT8 PlainText[],
		 ...
		 IN UINT8 Key[],
		 IN UINT KeyLength,
		 OUT UINT8 CipherText[], OUT UINT * CipherTextLength)
{
	...
	os_alloc_mem(NULL, (UCHAR **) & pResult,
...
*/
	...
	for (Index_j = 0; Index_j < 6; Index_j++) {
		for (Index_i = 0; Index_i < Number_Of_Block; Index_i++) {
			...
			NdisMoveMemory(Block_Input + 8, pResult + (Index_i * 8),
				       8);
			Temp_Length = sizeof(Block_B);
			RT_AES_Encrypt(Block_Input, AES_BLOCK_SIZES, Key,
				       KeyLength, Block_B, &Temp_Length);

			NdisMoveMemory(IV, Block_B, 8);
			IV[7] =
			    Block_B[7] ^ ((Number_Of_Block * Index_j) +
					  Index_i + 1);
			NdisMoveMemory(pResult + (Index_i * 8), (Block_B + 8),
				       8);
		}		/* End of for */
	}			/* End of for */
	...
	*CipherTextLength = PlainTextLength + AES_KEY_WRAP_IV_LENGTH;
	...
	NdisMoveMemory(CipherText + AES_KEY_WRAP_IV_LENGTH, pResult,
		       PlainTextLength);
...
INT AES_Key_Unwrap(IN UINT8 CipherText[],
		   ...
		   IN UINT8 Key[],
		   IN UINT KeyLength,
		   OUT UINT8 PlainText[], OUT UINT * PlainTextLength)
{
	UINT8 IV[8], Block_B[16], Block_Input[16];
	UINT8 *pResult;
...
*/
	...
	NdisMoveMemory(pResult, CipherText + AES_KEY_WRAP_IV_LENGTH,
		       PlainLength);
	...
	for (Index_j = 5; Index_j >= 0; Index_j--) {
		for (Index_i = (Number_Of_Block - 1); Index_i >= 0; Index_i--) {
			IV[7] =
			    IV[7] ^ ((Number_Of_Block * Index_j) + Index_i + 1);
			NdisMoveMemory(Block_Input, IV, 8);
			NdisMoveMemory(Block_Input + 8, pResult + (Index_i * 8),
				       8);
			Temp_Length = sizeof(Block_B);
			RT_AES_Decrypt(Block_Input, AES_BLOCK_SIZES, Key,
				       KeyLength, Block_B, &Temp_Length);
