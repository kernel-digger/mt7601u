...
#ifdef CONFIG_STA_SUPPORT
#ifdef PROFILE_STORE
static void WriteConfToDatFile(IN PRTMP_ADAPTER pAd)
{
	char *cfgData = 0;
...
//      INT                             tempStrLen = 0;
	...
	if (IS_FILE_OPEN_ERR(file_w)) {
		...
		RtmpOSFileWrite(file_w, (PSTRING) cfgData,
				(int)(offset - cfgData));
		...
		for (;;) {
			...
			if ((size_t) (offset - cfgData) < fileLen) {
				...
				if (strncmp(pTempStr, "SSID=", strlen("SSID="))
				    == 0) {
					NdisZeroMemory(pTempStr, 512);
					NdisMoveMemory(pTempStr, "SSID=",
						       strlen("SSID="));
					NdisMoveMemory(pTempStr + 5,
						       pAd->CommonCfg.Ssid,
				...
				RtmpOSFileWrite(file_w, pTempStr,
						strlen(pTempStr));
...
NDIS_STATUS WriteDatThread(IN RTMP_ADAPTER * pAd)
{
	...
	status =
	    RtmpOSTaskAttach(pTask, write_dat_file_thread,
			     (ULONG) & pAd->WriteDatTask);
	DBGPRINT(RT_DEBUG_TRACE,
		 ("<--WriteDatThreadInit(), status=%d!\n", status));
