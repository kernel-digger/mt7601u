...
VOID RtmpOsMsDelay(IN ULONG msec);

void RTMP_GetCurrentSystemTime(IN LARGE_INTEGER * time);

void RTMP_GetCurrentSystemTick(IN ULONG * pNow);

VOID RtmpOsWait(IN UINT32 Time);

UINT32 RtmpOsTimerAfter(IN ULONG a, IN ULONG b);

UINT32 RtmpOsTimerBefore(IN ULONG a, IN ULONG b);
...
NDIS_STATUS os_alloc_mem_suspend(IN VOID * pReserved,
...
NDIS_STATUS AdapterBlockAllocateMemory(IN PVOID handle,
				       OUT PVOID * ppAd, IN UINT32 SizeOfpAd);

VOID *RtmpOsVmalloc(IN ULONG Size);

VOID RtmpOsVfree(IN VOID * pMem);

ULONG RtmpOsCopyFromUser(OUT VOID * to, IN const void *from, IN ULONG n);

ULONG RtmpOsCopyToUser(OUT VOID * to, IN const void *from, IN ULONG n);

BOOLEAN RtmpOsStatsAlloc(IN VOID ** ppStats, IN VOID ** ppIwStats);

/* OS Packet */
PNDIS_PACKET RtmpOSNetPktAlloc(IN VOID * pReserved, IN int size);

PNDIS_PACKET RTMP_AllocateFragPacketBuffer(IN VOID * pReserved,
					   IN ULONG Length);
...
VOID RTMPFreeNdisPacket(IN VOID * pReserved, IN PNDIS_PACKET pPacket);
...
PNDIS_PACKET DuplicatePacket(IN PNET_DEV pNetDev,
...
PNDIS_PACKET duplicate_pkt_with_TKIP_MIC(IN VOID * pReserved,
...
PNDIS_PACKET duplicate_pkt_with_VLAN(IN PNET_DEV pNetDev,
				     ...
				     IN PUCHAR pData,
				     IN ULONG DataSize,
				     IN UCHAR FromWhichBSSID, IN UCHAR * TPID);
...
BOOLEAN RTMPL2FrameTxAction(IN VOID * pCtrlBkPtr,
			    IN PNET_DEV pNetDev,
			    IN RTMP_CB_8023_PACKET_ANNOUNCE
			    _announce_802_3_packet, IN UCHAR apidx,
			    IN PUCHAR pData, IN UINT32 data_len,
			    IN UCHAR OpMode);

PNDIS_PACKET ExpandPacket(IN VOID * pReserved,
			  IN PNDIS_PACKET pPacket,
			  IN UINT32 ext_head_len, IN UINT32 ext_tail_len);
...
VOID RtmpOsSetPacket(IN PNET_DEV pNetDev,
		     IN PNDIS_PACKET pRxPacket,
		     IN UCHAR * pData, IN ULONG DataSize);
...
VOID RtmpOsPktBodyCopy(IN PNET_DEV pNetDev,
		       ...
		       IN ULONG ThisFrameLen, IN PUCHAR pData);
...
VOID RtmpOsPktDataPtrAssign(IN PNDIS_PACKET pNetPkt, IN UCHAR * pData);

VOID RtmpOsPktLenAssign(IN PNDIS_PACKET pNetPkt, IN LONG Len);

VOID RtmpOsPktTailAdjust(IN PNDIS_PACKET pNetPkt, IN UINT removedTagLen);

PUCHAR RtmpOsPktTailBufExtend(IN PNDIS_PACKET pNetPkt, IN UINT Len);

PUCHAR RtmpOsPktHeadBufExtend(IN PNDIS_PACKET pNetPkt, IN UINT Len);

VOID RtmpOsPktReserve(IN PNDIS_PACKET pNetPkt, IN UINT Len);
...
void RtmpOSFSInfoChange(IN RTMP_OS_FS_INFO * pOSFSInfoOrg, IN BOOLEAN bSet);

/* OS Network Interface */
int RtmpOSNetDevAddrSet(IN UCHAR OpMode,
			IN PNET_DEV pNetDev,
			IN PUCHAR pMacAddr, IN PUCHAR dev_name);
...
INT RtmpOSNotifyRawData(IN PNET_DEV pNetDev,
			IN PUCHAR buff,
			IN INT len, IN ULONG type, IN USHORT protocol);
...
INT RtmpOSNetDevDestory(IN VOID * pReserved, IN PNET_DEV pNetDev);
...
VOID RtmpOsSetPktNetDev(IN VOID * pPkt, IN VOID * pDev);

PNET_DEV RtmpOsPktNetDevGet(IN VOID * pPkt);

char *RtmpOsGetNetDevName(IN VOID * pDev);

VOID RtmpOsSetNetDevPriv(IN VOID * pDev, IN VOID * pPriv);
...
BOOLEAN RtmpOSTaskAlloc(IN RTMP_OS_TASK * pTask, IN LIST_HEADER * pTaskList);
...
BOOLEAN RtmpOSTaskWait(IN VOID * pReserved,
		       IN RTMP_OS_TASK * pTaskOrg, IN INT32 * pStatus);
...
VOID RTMP_OS_Init_Timer(IN VOID * pReserved,
			...
			IN PVOID data, IN LIST_HEADER * pTimerList);
...
int RtmpOsPciConfigReadWord(IN VOID * pDev,
			    IN UINT32 Offset, OUT UINT16 * pValue);

int RtmpOsPciConfigWriteWord(IN VOID * pDev, IN UINT32 Offset, IN UINT16 Value);

int RtmpOsPciConfigReadDWord(IN VOID * pDev,
			     IN UINT32 Offset, OUT UINT32 * pValue);

int RtmpOsPciConfigWriteDWord(IN VOID * pDev,
			      IN UINT32 Offset, IN UINT32 Value);

int RtmpOsPciFindCapability(IN VOID * pDev, IN int Cap);
...
typedef VOID(*RTMP_OS_SEND_WLAN_EVENT) (IN VOID * pAdSrc,
					...
					IN PUCHAR pAddr,
					IN UCHAR BssIdx, IN CHAR Rssi);

VOID RtmpOsSendWirelessEvent(IN VOID * pAd,
			     IN USHORT Event_flag,
			     IN PUCHAR pAddr,
			     IN UCHAR BssIdx,
			     IN CHAR Rssi, IN RTMP_OS_SEND_WLAN_EVENT pFunc);
...
int RtmpOSWrielessEventSendExt(IN PNET_DEV pNetDev,
			       ...
			       IN PUCHAR pSrcMac,
			       IN PUCHAR pData,
			       IN UINT32 dataLen, IN UINT32 family);
...
VOID RtmpDrvAllMacPrint(IN VOID * pReserved,
			IN UINT32 * pBufMac,
			IN UINT32 AddrStart,
			IN UINT32 AddrEnd, IN UINT32 AddrStep);
...
int RtmpOSIRQRelease(IN PNET_DEV pNetDev,
		     IN UINT32 infType,
		     IN PPCI_DEV pci_dev, IN BOOLEAN * pHaveMsi);

VOID RtmpOsWlanEventSet(IN VOID * pReserved,
			IN BOOLEAN * pCfgWEnt, IN BOOLEAN FlgIsWEntSup);
...
long RtmpOsSimpleStrtol(IN const char *cp,
			IN char **endp, IN unsigned int base);

VOID RtmpOsOpsInit(RTMP_OS_ABL_OPS * pOps);

/* ============================ rt_os_util.c ================================ */
VOID RtmpDrvMaxRateGet(IN VOID * pReserved,
		       IN UINT8 MODE,
		       IN UINT8 ShortGI,
		       IN UINT8 BW, IN UINT8 MCS, OUT UINT32 * pRate);
...
VOID WpaSendMicFailureToWpaSupplicant(IN PNET_DEV pNetDev, IN BOOLEAN bUnicast);

int wext_notify_event_assoc(IN PNET_DEV pNetDev,
			    IN UCHAR * ReqVarIEs, IN UINT32 ReqVarIELen);

VOID SendAssocIEsToWpaSupplicant(IN PNET_DEV pNetDev,
				 IN UCHAR * ReqVarIEs, IN UINT32 ReqVarIELen);
...
void linux_pci_unmap_single(void *pPciDev, ra_dma_addr_t dma_addr, size_t size,
			    int direction);
...
BOOLEAN CFG80211_SupBandInit(IN VOID * pCB,
			     ...
			     IN VOID * pWiphyOrg,
			     IN VOID * pChannelsOrg, IN VOID * pRatesOrg);

BOOLEAN CFG80211OS_SupBandReInit(IN VOID * pCB, IN CFG80211_BAND * pBandInfo);

VOID CFG80211OS_RegHint(IN VOID * pCB,
			IN UCHAR * pCountryIe, IN ULONG CountryIeLen);

VOID CFG80211OS_RegHint11D(IN VOID * pCB,
			   IN UCHAR * pCountryIe, IN ULONG CountryIeLen);

BOOLEAN CFG80211OS_BandInfoGet(IN VOID * pCB,
			       IN VOID * pWiphyOrg,
			       OUT VOID ** ppBand24, OUT VOID ** ppBand5);

UINT32 CFG80211OS_ChanNumGet(IN VOID * pCB,
			     IN VOID * pWiphyOrg, IN UINT32 IdBand);
...
BOOLEAN CFG80211OS_ChanInfoInit(IN VOID * pCB,
				...
				IN UCHAR ChanId,
				IN UCHAR MaxTxPwr,
				IN BOOLEAN FlgIsNMode, IN BOOLEAN FlgIsBW20M);

VOID CFG80211OS_Scaning(IN VOID * pCB,
			IN UINT32 ChanId,
			IN UCHAR * pFrame,
			IN UINT32 FrameLen,
			IN INT32 RSSI, IN BOOLEAN FlgIsNMode, IN UINT8 BW);
