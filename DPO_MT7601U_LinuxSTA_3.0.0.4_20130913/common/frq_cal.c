...
VOID InitFrequencyCalibration(IN PRTMP_ADAPTER pAd)
{
	if (pAd->FreqCalibrationCtrl.bEnableFrequencyCalibration == TRUE) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s: frequency offset in the EEPROM = %ld(0x%x)\n",
			  __FUNCTION__, pAd->RfFreqOffset, pAd->RfFreqOffset));
...
VOID FrequencyCalibrationMode(PRTMP_ADAPTER pAd, UINT8 Mode)
{
	...
	if (Mode == FREQ_CAL_MODE2) {
		rlt_rf_write(pAd, RF_BANK0, RF_R12,
			     pAd->FreqCalibrationCtrl.AdaptiveFreqOffset);
...
VOID FrequencyCalibration(IN PRTMP_ADAPTER pAd)
{
	...
	if ((pAd->FreqCalibrationCtrl.bEnableFrequencyCalibration == TRUE) &&
	    (((pAd->FreqCalibrationCtrl.bApproachFrequency == FALSE)
	      && ((pAd->Mlme.PeriodicRound % FREQUENCY_CALIBRATION_PERIOD) ==
		  0)) || ((pAd->FreqCalibrationCtrl.bApproachFrequency == TRUE)
			  &&
			  ((pAd->Mlme.PeriodicRound %
			    (FREQUENCY_CALIBRATION_PERIOD / 20)) == 0)))) {
		DBGPRINT(RT_DEBUG_INFO, ("---> %s\n", __FUNCTION__));

		if (pAd->FreqCalibrationCtrl.bSkipFirstFrequencyCalibration ==
		    TRUE) {
			pAd->
			    FreqCalibrationCtrl.bSkipFirstFrequencyCalibration =
			    FALSE;
			...
			if (pAd->
			    FreqCalibrationCtrl.LatestFreqOffsetOverBeacon !=
			    INVALID_FREQUENCY_OFFSET) {
				/* Sync the thresholds */
				if (pAd->FreqCalibrationCtrl.BeaconPhyMode == MODE_CCK) {	/* CCK */
					...
					if (IS_MT7601(pAd)) {
						HighFreqTriggerPoint =
						    MT7601_HIGH_FREQUENCY_TRIGGER_POINT_CCK;
						LowFreqTriggerPoint =
						    MT7601_LOW_FREQUENCY_TRIGGER_POINT_CCK;

						DecreaseFreqOffset =
						    MT7601_DECREASE_FREQUENCY_OFFSET_CCK;
						IncreaseFreqOffset =
						    MT7601_INCREASE_FREQUENCY_OFFSET_CCK;
					} else
#endif /* MT7601 */
					{
						HighFreqTriggerPoint =
						...
						DecreaseFreqOffset =
						    DECREASE_FREQUENCY_OFFSET_CCK;
						IncreaseFreqOffset =
						    INCREASE_FREQUENCY_OFFSET_CCK;
					}
				} else {	/* OFDM */
					...
					if (IS_MT7601(pAd)) {
						DBGPRINT(RT_DEBUG_ERROR,
							 ("%s:MT7601 receive OFDM beacon.\n",
							  __FUNCTION__));

						HighFreqTriggerPoint =
						    MT7601_HIGH_FREQUENCY_TRIGGER_POINT_OFDM20;
						LowFreqTriggerPoint =
						    MT7601_LOW_FREQUENCY_TRIGGER_POINT_OFDM20;

						DecreaseFreqOffset =
						    MT7601_DECREASE_FREQUENCY_OFFSET_OFDM20;
						IncreaseFreqOffset =
						    MT7601_INCREASE_FREQUENCY_OFFSET_OFDM20;
					} else
#endif /* MT7601 */
					{
						HighFreqTriggerPoint =
						...
						DecreaseFreqOffset =
						    DECREASE_FREQUENCY_OFFSET_OFDM;
						IncreaseFreqOffset =
						    INCREASE_FREQUENCY_OFFSET_OFDM;
					}
				}
				...
				if ((pAd->
					...
					<= LowFreqTriggerPoint)) {
					pAd->
					    FreqCalibrationCtrl.bApproachFrequency
					    = TRUE;
				}
...
#ifdef MT7601
						if (pAd->
						    FreqCalibrationCtrl.AdaptiveFreqOffset
						    > 0)
#endif /* MT7601 */
							pAd->FreqCalibrationCtrl.AdaptiveFreqOffset--;
						DBGPRINT(RT_DEBUG_TRACE,
...
inline CHAR GetFrequencyOffsetField(PRTMP_ADAPTER pAd,
				    RXWI_STRUC * pRxWI,
...
CHAR GetFrequencyOffset(IN PRTMP_ADAPTER pAd, IN RXWI_STRUC * pRxWI)
{
	...
	if (pAd->FreqCalibrationCtrl.bEnableFrequencyCalibration) {
		...
		FreqOffset =
		    GetFrequencyOffsetField(pAd, pRxWI,
					    pAd->chipCap.RxWIFrqOffset);
