...
UINT32 SwitchTime1, SwitchTime2, SwitchTime3, SwitchTime4, SwitchTime5,
    SwitchTime6, SwitchTime7, SwitchTime8;
			    ...
			    IN BOOLEAN bWaitACK, IN CHAR Index)
{
	...
	pNullFr->Duration =
	    pAd->CommonCfg.Dsifs + RTMPCalcDuration(pAd, pAd->CommonCfg.TxRate,
						    14);
	...
	RTMPWriteTxWI(pAd, pTxWI, FALSE, FALSE, FALSE, FALSE, TRUE, FALSE, 0,
		      pEntry->Aid, frameLen, 0, 0,
		      (UCHAR) pAd->CommonCfg.MlmeTransmit.field.MCS, IFS_HTTXOP,
		      FALSE, &pAd->CommonCfg.MlmeTransmit);
	...
	RTMPFrameEndianChange(pAd, (PUCHAR) pNullFrBuf + TXWISize, DIR_WRITE,
			      FALSE);
	...
	for (i = 0; i < totalLen; i += 4) {
		...
		if (Index == 0)
			RTMP_IO_WRITE32(pAd, pAd->NullBufOffset[0] + i,
					longValue);
		else if (Index == 1)
			RTMP_IO_WRITE32(pAd, pAd->NullBufOffset[1] + i,
					longValue);
			 ...
			 IN BOOLEAN bQosNull, IN USHORT PwrMgmt, IN CHAR Index)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s - Send NULL Frame @%d Mbps...%d \n", __FUNCTION__,
		  RateIdToMbps[pAd->CommonCfg.TxRate], PwrMgmt));
	...
	if (Index == 0) {
		...
		longValue =
		    *ptr + (*(ptr + 1) << 8) + (*(ptr + 2) << 16) +
		    (*(ptr + 3) << 24);
#endif /* RT_BIG_ENDIAN */
		RTMP_IO_WRITE32(pAd, pAd->NullBufOffset[0] + TXWISize,
				longValue);
		...
		longValue =
		    *ptr + (*(ptr + 1) << 8) + (*(ptr + 2) << 16) +
		    (*(ptr + 3) << 24);
		RTMP_IO_WRITE32(pAd, pAd->NullBufOffset[0] + TXWISize + 20,
				longValue);
		...
		longValue =
		    *ptr + (*(ptr + 1) << 8) + (*(ptr + 2) << 16) +
		    (*(ptr + 3) << 24);
#endif /* RT_BIG_ENDIAN */
		RTMP_IO_WRITE32(pAd, pAd->NullBufOffset[1] + TXWISize,
				longValue);
		...
		longValue =
		    *ptr + (*(ptr + 1) << 8) + (*(ptr + 2) << 16) +
		    (*(ptr + 3) << 24);
		RTMP_IO_WRITE32(pAd, pAd->NullBufOffset[1] + TXWISize + 20,
				longValue);
...
VOID RtmpEnqueueLastNullFrame(IN PRTMP_ADAPTER pAd,
			      ...
			      IN BOOLEAN bQosNull,
			      IN BOOLEAN bEOSP,
			      IN UCHAR OldUP, IN UCHAR PwrMgmt, IN UCHAR OpMode)
{
	UCHAR NullFrame[48];
	ULONG Length;
...
static VOID MACBuffer_Change(RTMP_ADAPTER * pAd,
			     BOOLEAN hcca_to_edca, BOOLEAN edca_to_hcca)
{
	MAC_TABLE_ENTRY *pEntry = NULL;
	PAPCLI_STRUCT pApCliEntry = NULL;
	UINT i = 0;
...
//      RTMP_SEM_EVENT_UP(&pAd->reg_atomic2);   
	...
	if (edca_to_hcca)
		RTMPHwSendNullFrame(pAd,
				    ...
				    (pAd->CommonCfg.
				     bWmmCapable & pAd->CommonCfg.APEdcaParm.
				     bValid), PWR_SAVE, 0);
	...
	if (hcca_to_edca) {
		...
		AsicSetChannel(pAd, pAd->CommonCfg.CentralChannel,
			       pAd->StaCfg.BW, ext_ch, FALSE);

	}

	if (edca_to_hcca) {
		if (pAd->P2pCfg.BW == BW_40) {
			if (pAd->ApCliMlmeAux.CentralChannel >
		...
		AsicSetChannel(pAd, pAd->ApCliMlmeAux.CentralChannel,
			       pAd->P2pCfg.BW, ext_ch, FALSE);
	...
	if ((i == 10) || (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST))) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Multi Channel Switch Retry count exhausted\n"));
	}
...
static VOID ProcessEDCAToHCCA(RTMP_ADAPTER * pAd)
{
	PAPCLI_STRUCT pApCliEntry = NULL;
	...
	if ((pApCliEntry->Valid) && INFRA_ON(pAd)) {
		...
		RTMPSetTimer(&pAd->Mlme.MCCTimer,
			     pAd->Mlme.HCCAToEDCATimerValue);
	}
}

static VOID ProcessHCCAToEDCA(PRTMP_ADAPTER pAd)
{
	UINT32 MacValue;
	UINT32 i = 0;
	INT ret;
...
static INT MultiChannelTaskThread(IN ULONG Context)
{
	RTMP_OS_TASK *pTask;
	RTMP_ADAPTER *pAd;
	INT Status = 0;
...
NDIS_STATUS MultiChannelThreadInit(IN PRTMP_ADAPTER pAd)
{
	...
	RTMPInitTimer(pAd, &pAd->Mlme.MCCTimer,
		      GET_TIMER_FUNCTION(MCC_ChangeAction), pAd, FALSE);
	RTMPInitTimer(pAd, &pAd->Mlme.ConcurrentP2PConnectTimer,
		      GET_TIMER_FUNCTION(ConcurrentP2PConnectTimeout), pAd,
		      FALSE);

	RTMP_OS_TASK_INIT(pTask, "MultiChannelTask", pAd);
	status =
	    RtmpOSTaskAttach(pTask, MultiChannelTaskThread,
			     (ULONG) & pAd->MultiChannelTask);
	DBGPRINT(RT_DEBUG_TRACE,
		 ("<-- %s, status=%d!\n", __FUNCTION__, status));
...
BOOLEAN MultiChannelThreadExit(IN PRTMP_ADAPTER pAd)
{
	...
	if (ret == NDIS_STATUS_FAILURE) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s: kill multi-channel task failed!\n",
			  __FUNCTION__));
	}
	return TRUE;
}
...
VOID MultiChannelTimerStart(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (pAd->P2pCfg.bStartP2pConnect) {
		...
		if (pAd->Mlme.ConcurrentP2PConnectTimerRunning) {
			...
			RTMPCancelTimer(&pAd->Mlme.ConcurrentP2PConnectTimer,
					&bCancelled);
			pAd->Mlme.ConcurrentP2PConnectTimerRunning = FALSE;
		}
	}
...
//      MACBuffer_Change(pAd,TRUE, FALSE);
	...
	AsicSetChannel(pAd, pAd->CommonCfg.CentralChannel, pAd->StaCfg.BW,
		       ext_ch, FALSE);

	RTMPHwSendNullFrame(pAd,
			    pAd->CommonCfg.TxRate,
			    (pAd->CommonCfg.bWmmCapable & pAd->
			     CommonCfg.APEdcaParm.bValid), PWR_ACTIVE, 0);
...
VOID MultiChannelSwitchToP2P(IN PRTMP_ADAPTER pAd)
{
	...
	AsicSetChannel(pAd, pAd->ApCliMlmeAux.CentralChannel, pAd->P2pCfg.BW,
		       ext_ch, FALSE);

	RTMPHwSendNullFrame(pAd,
			    pAd->CommonCfg.TxRate,
			    (pAd->CommonCfg.bWmmCapable & pAd->
			     CommonCfg.APEdcaParm.bValid), PWR_ACTIVE, 1);
