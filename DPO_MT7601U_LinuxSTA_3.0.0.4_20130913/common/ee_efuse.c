...
static UCHAR eFuseReadRegisters(IN PRTMP_ADAPTER pAd,
				IN USHORT Offset,
...
static VOID eFuseWritePhysical(IN PRTMP_ADAPTER pAd,
			       PUSHORT lpInBuffer,
			       ULONG nInBufferSize,
			       PUCHAR lpOutBuffer, ULONG nOutBufferSize);

static NTSTATUS eFuseWriteRegistersFromBin(IN PRTMP_ADAPTER pAd,
					   IN USHORT Offset,
					   IN USHORT Length, IN USHORT * pData);
...
========================================================================
*/
NTSTATUS eFuseRead(IN PRTMP_ADAPTER pAd,
		   IN USHORT Offset, OUT PUSHORT pData, IN USHORT Length)
{
					...
					IN USHORT Length, OUT USHORT * pData)
{
	...
	for (i = 0; i < 4; i++) {
		RTMP_IO_READ32(pAd, efuseDataOffset,
	...
	if (EFSROM_AOUT == 0x3f) {	/*find available logical address pointer        */
		...
		for (i = StartBlock; i >= EndBlock; i -= 2) {
			...
			if ((LogicalAddress & 0xff) == 0) {	/*Not used logical address pointer */
				if (i !=
				    (pAd->chipCap.EFUSE_USAGE_MAP_START - 1)) {
					BlkNum =
					    i -
					    pAd->chipCap.EFUSE_USAGE_MAP_START;
					break;
		...
		tmpOffset |=
		    ((~
		      ((tmpOffset & 0x01) ^ (tmpOffset >> 1 & 0x01) ^
		       (tmpOffset >> 2 & 0x01) ^ (tmpOffset >> 3 & 0x01))) << 6)
		    & 0x40;
		tmpOffset |=
		    ((~
		      ((tmpOffset >> 2 & 0x01) ^ (tmpOffset >> 3 & 0x01) ^
		       (tmpOffset >> 4 & 0x01) ^ (tmpOffset >> 5 & 0x01))) << 7)
		    & 0x80;
...
========================================================================
*/
NTSTATUS eFuseWrite(IN PRTMP_ADAPTER pAd,
		    IN USHORT Offset, IN PUSHORT pData, IN USHORT length)
{
	int i;
...
/*	memPtr = kmalloc(memSize, MEM_ALLOC_FLAG);*/
	...
	if (strlen(arg) > 0)
		...
		NdisMoveMemory(src, EFUSE_EEPROM_DEFULT_FILE,
			       strlen(EFUSE_EEPROM_DEFULT_FILE));
	...
	for (ReadedByte = 0; ReadedByte < TotalByte; ReadedByte++) {
		...
		if ((ReadedByte + 1) % 2 == 0)
			PDATA[ReadedByte / 2 % 8] =
			    ((buffer[ReadedByte] << 8) & 0xff00) |
			    (buffer[ReadedByte - 1] & 0xff);
		if (ReadedByte % 16 == 0) {
			...
			if ((ReadedByte + 1) % 16 == 0) {
				...
				if (CompareBuf != 0xff)
					eFuseWriteRegistersFromBin(pAd, (USHORT)
								   ReadedByte -
								   15, 16,
								   PDATA);
				else {
					if (eFuseReadRegisters
					    (pAd, ReadedByte, 2,
					     (PUSHORT) & DATA) != 0x3f)
						eFuseWriteRegistersFromBin(pAd,
									   (USHORT)
									   ReadedByte
									   - 15,
									   16,
					   ...
					   IN USHORT Length, IN USHORT * pData)
{
	...
	do {
		...
		if (bAllocateNewBlk) {
			...
			for (i = 0; i < 4; i++) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Allocate New Blk, Data%d=%04x%04x\n",
					  ...
					  pData[2 * i]));
				tempbuffer =
				    ((pData[2 * i + 1] << 16) & 0xffff0000) |
				    pData[2 * i];

				RTMP_IO_WRITE32(pAd, efuseDataOffset,
						tempbuffer);
				efuseDataOffset -= 4;
			...
			i = 0;
			while (i < 100) {
				RTMP_IO_READ32(pAd, EFUSE_CTRL,
					       (PUINT32) & eFuseCtrlStruc);
			...
			for (i = 0; i < 4; i++) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("buffer[%d]=%x,pData[%d]=%x,pData[%d]=%x,tempbuffer=%x\n",
					  i, buffer[i], 2 * i, pData[2 * i],
					  2 * i + 1, pData[2 * i + 1],
					  tempbuffer));

				if (((buffer[i] & 0xffff0000) ==
				     (pData[2 * i + 1] << 16))
				    && ((buffer[i] & 0xffff) == pData[2 * i]))
					bNotWrite &= TRUE;
				else {
					bNotWrite &= FALSE;
					break;
			...
			if (!bNotWrite) {
				...
				for (i = 0; i < 8; i++) {
					...
					eFuseWritePhysical(pAd, &InBuf[0], 6,
							   NULL, 2);
				}
...
int rtmp_ee_efuse_write16(IN RTMP_ADAPTER * pAd,
...
INT set_eFuseBufferModeWriteBack_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (Enable == 1) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("set_eFuseBufferMode_Proc:: Call WRITEEEPROMBUF"));
...
/*
	...
	========================================================================
*/
INT eFuseLoadEEPROM(IN PRTMP_ADAPTER pAd)
{
	PSTRING src = NULL;
	INT retval;
	RTMP_OS_FD srcf;
...
INT eFuseWriteEeeppromBuf(IN PRTMP_ADAPTER pAd)
{
	...
	if (src && *src) {
		...
		if (IS_FILE_OPEN_ERR(srcf)) {
			...
			RtmpOSFileWrite(srcf, (PSTRING) pAd->EEPROMImage,
					MAX_EEPROM_BIN_FILE_SIZE);
...
INT eFuse_init(RTMP_ADAPTER * pAd)
{
	...
	if (EfuseFreeBlock > (pAd->chipCap.EFUSE_USAGE_MAP_SIZE - 5)) {
		...
		if (ret == FALSE) {
			if (pAd->chipCap.EFUSE_DEFAULT_BIN != NULL) {
				NdisMoveMemory(pAd->EEPROMImage,
					       pAd->chipCap.EFUSE_DEFAULT_BIN,
					       pAd->
					       chipCap.EFUSE_DEFAULT_BIN_SIZE);
			}
		}
#ifdef MT7601
		if (IS_MT7601(pAd)) {
			...
			eFuseReadRegisters(pAd, EEPROM_FREQ_OFFSET, 2,
					   &FrequencyOffset);
			eFuseReadRegisters(pAd, EEPROM_NIC1_OFFSET, 2,
					   &NicConfig);

			if ((NicConfig == 0x0) && (FrequencyOffset != 0xFFFF)) {
				/* Calibration Free IC, but E-Fuse is empty */
				DBGPRINT(RT_DEBUG_OFF,
					 ("Calibration Free IC, Load calibration data...\n"));

				pAd->EEPROMImage[EEPROM_FREQ_OFFSET] =
				    (UCHAR) (FrequencyOffset & 0xFF);

				eFuseReadRegisters(pAd, EEPROM_TX0_TSSI_SLOPE,
						   2, &EfuseValue);
				*(UINT16
				  *) (&pAd->EEPROMImage[EEPROM_TX0_TSSI_SLOPE])
				    = EfuseValue;
				eFuseReadRegisters(pAd,
						   EEPROM_TX0_TSSI_OFFSET_GROUP1,
						   2, &EfuseValue);
				*(UINT16 *) (&pAd->EEPROMImage
					     [EEPROM_TX0_TSSI_OFFSET_GROUP1]) =
				    EfuseValue;
				eFuseReadRegisters(pAd, EEPROM_TX0_TSSI_OFFSET,
						   2, &EfuseValue);
				pAd->EEPROMImage[EEPROM_TX0_TSSI_OFFSET] =
				    (UCHAR) (EfuseValue & 0xFF);;

				eFuseReadRegisters(pAd, EEPROM_G_TARGET_POWER,
						   2, &EfuseValue);
				pAd->EEPROMImage[EEPROM_G_TARGET_POWER + 1] =
				    (UCHAR) (EfuseValue >> 8);
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("NVM is Efuse and force to use EEPROM Buffer Mode=%x\n",
...
INT Set_LoadEepromBufferFromEfuse_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (bEnable < 0)
		...
		eFuseRead(pAd, 0, (PUSHORT) & pAd->EEPROMImage[0],
			  MAX_EEPROM_BIN_FILE_SIZE);
