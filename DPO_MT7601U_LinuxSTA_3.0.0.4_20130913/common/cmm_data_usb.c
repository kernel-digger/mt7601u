...
/*
	...
	========================================================================
*/
static VOID rlt_usb_write_txinfo(IN RTMP_ADAPTER * pAd,
				 IN TXINFO_STRUC * pTxInfo,
				 IN USHORT USBDMApktLen,
				 IN BOOLEAN bWiv,
				 IN UCHAR QueueSel,
				 IN UCHAR NextValid,
				 IN UCHAR TxBurst, IN UCHAR pkt_80211)
{
#ifdef RLT_MAC
...
static VOID rlt_usb_update_txinfo(IN RTMP_ADAPTER * pAd,
				  IN TXINFO_STRUC * pTxInfo, IN TX_BLK * pTxBlk)
{
#ifdef RLT_MAC
#endif /* RLT_MAC */
}
...
VOID ComposePsPoll(RTMP_ADAPTER * pAd)
{
	...
	pAd->PsPollContext.BulkOutSize =
	    TXINFO_SIZE + TXWISize + TSO_SIZE + data_len + 4;
}
...
VOID ComposeNullFrame(RTMP_ADAPTER * pAd)
{
	...
	RTMPWriteTxWI(pAd, pTxWI, FALSE, FALSE, FALSE, FALSE, TRUE, FALSE, 0,
		      ...
		      (UCHAR) pAd->CommonCfg.MlmeTransmit.field.MCS,
		      IFS_BACKOFF, FALSE, &pAd->CommonCfg.MlmeTransmit);
	RTMPMoveMemory((VOID *) & buf[TXWISize + TXINFO_SIZE],
		       (VOID *) & pAd->NullFrame, data_len);
					    ...
					    IN HT_TX_CONTEXT * pHTTXContext)
{
	...
	if (((pHTTXContext->CurWriteIdx < pHTTXContext->NextBulkIdx)
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("RtmpUSBCanDoWrite USB_BULK_BUF_ALIGMENT c1!\n"));
		RTUSB_SET_BULK_FLAG(pAd,
				    (fRTUSB_BULK_OUT_DATA_NORMAL << QueIdx));
	} else if (pHTTXContext->bCurWriting == TRUE) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("RtmpUSBCanDoWrite USB_BULK_BUF_ALIGMENT c3!!\n"));
	...
	if (((pHTTXContext->CurWritePosition) <
		...
		DBGPRINT(RT_DEBUG_ERROR, ("RtmpUSBCanDoWrite c3!\n"));
		   ...
		   && ((pHTTXContext->CurWritePosition + 7912) >
		       MAX_TXBULK_LIMIT)) {
...
USHORT RtmpUSB_WriteSubTxResource(IN RTMP_ADAPTER * pAd,
				  IN TX_BLK * pTxBlk,
				  IN BOOLEAN bIsLast, OUT USHORT * freeCnt)
{
...
USHORT RtmpUSB_WriteFragTxResource(IN RTMP_ADAPTER * pAd,
				   IN TX_BLK * pTxBlk,
				   IN UCHAR fragNum, OUT USHORT * freeCnt)
{
	HT_TX_CONTEXT *pHTTXContext;
	...
	if (fragNum == pTxBlk->TotalFragNum) {
		...
		if (((pHTTXContext->CurWritePosition + 3906) & 0x00006000) ==
		    0x00006000) {
	...
	NdisMoveMemory(pWirelessPacket, pTxBlk->HeaderBuf,
		       TXINFO_SIZE + TXWISize + hwHdrLen);
#ifdef RT_BIG_ENDIAN
	RTMPFrameEndianChange(pAd,
			      (PUCHAR) (pWirelessPacket + TXINFO_SIZE +
					TXWISize), DIR_WRITE, FALSE);
#endif /* RT_BIG_ENDIAN */
				     ...
				     IN BOOLEAN bIsLast, OUT USHORT * freeCnt)
{
	...
	if (Status == NDIS_STATUS_SUCCESS) {
		...
		/* Build our URB for USBD */
		hdr_len =
		    TXWISize + TSO_SIZE + pTxBlk->MpduHeaderLen +
		    pTxBlk->HdrPadLen;
				...
				if (len <= 0) {
					DBGPRINT(RT_DEBUG_ERROR,
						 ("%s():sg[%d] info error, sg.data=0x%x, sg.len=%d, pTxBlk->pSrcBufData=0x%x, pTxBlk->SrcBufLen=%d, data=0x%x, len=%d\n",
						  __FUNCTION__, i, sg[i].data,
						  sg[i].len,
						  pTxBlk->pSrcBufData,
						  pTxBlk->SrcBufLen, data,
						  len));
					break;
				}
...
USHORT RtmpUSB_WriteMultiTxResource(IN RTMP_ADAPTER * pAd,
				    IN TX_BLK * pTxBlk,
				    IN UCHAR frmNum, OUT USHORT * freeCnt)
{
	HT_TX_CONTEXT *pHTTXContext;
	USHORT hwHdrLen;	/* The hwHdrLen consist of 802.11 header length plus the header padding length. */
	UINT32 fillOffset;
...
VOID RtmpUSBDataLastTxIdx(IN RTMP_ADAPTER * pAd,
...
*/
VOID RtmpUSBDataKickOut(IN RTMP_ADAPTER * pAd,
...
int RtmpUSBMgmtKickOut(IN RTMP_ADAPTER * pAd,
		       ...
		       IN UCHAR * pSrcBufVA, IN UINT SrcBufLen)
{
	TXINFO_STRUC *pTxInfo;
	ULONG BulkOutSize;
	UCHAR padLen;
	...
	rlt_usb_write_txinfo(pAd, pTxInfo, (USHORT) (BulkOutSize - TXINFO_SIZE),
			     TRUE, EpToQueue[MGMTPIPEIDX], FALSE, FALSE, 1);
...
//+++Add by shiang for debug
	if (0) {
		DBGPRINT(RT_DEBUG_OFF,
			 ("-->%s():shiang-6590, QueIdx=%d, SrcBufLen=%d\n",
			     ...
			     IN UCHAR * pNullFrame, IN UINT32 frameLen)
{
	...
	if (pNullContext->InUse == FALSE) {
		...
		rlt_usb_write_txinfo(pAd, pTxInfo,
				     (USHORT) (frameLen + TXWISize + TSO_SIZE),
		...
		RTMPWriteTxWI(pAd, pTxWI, FALSE, FALSE, FALSE, FALSE, TRUE,
			      FALSE, 0, BSSID_WCID, frameLen, 0, 0,
			      (UCHAR) pAd->CommonCfg.MlmeTransmit.field.MCS,
			      IFS_HTTXOP, FALSE, &pAd->CommonCfg.MlmeTransmit);
#ifdef RT_BIG_ENDIAN
		RTMPWIEndianChange(pAd, (PUCHAR) pTxWI, TYPE_TXWI);
#endif /* RT_BIG_ENDIAN */
		RTMPMoveMemory(&pWirelessPkt[TXWISize + TXINFO_SIZE + TSO_SIZE],
			       pNullFrame, frameLen);
#ifdef RT_BIG_ENDIAN
		RTMPFrameEndianChange(pAd,
				      (PUCHAR) & pWirelessPkt[TXINFO_SIZE +
							      TXWISize +
							      TSO_SIZE],
				      DIR_WRITE, FALSE);
#endif /* RT_BIG_ENDIAN */
		pNullContext->BulkOutSize =
		    TXINFO_SIZE + TXWISize + TSO_SIZE + frameLen + 4;
		...
		if ((QueIdx == HCCA_PIPE)
		    && (pAd->Multi_Channel_Enable == TRUE))
			RTUSB_SET_BULK_FLAG(pAd,
					    fRTUSB_BULK_OUT_DATA_NULL_HCCA);
		else
#endif /* CONFIG_MULTI_CHANNEL */
...
//---Add by shiang for debug
	...
	/* 复制数据到skb->data */
	/* copy the rx packet */
	RTMP_USB_PKT_COPY(get_netdev_from_bssid(pAd, BSS0), pNetPkt,
			  ThisFrameLen, pData);
			     ...
			     IN RXWI_STRUC * pRxWI, IN RXINFO_STRUC * pRxInfo)
{
	...
	if (pRxInfo->Decrypted && pRxInfo->CipherErr) {
		...
		if (pRxInfo->Decrypted &&
		    (pAd->SharedKey[BSS0][pRxWI->RxWIKeyIndex].CipherAlg ==
		     CIPHER_AES)
	...
	if (pAd && pAd->Mlme.AutoWakeupTimerRunning) {
		...
		if (IS_MT7601(pAd)) {
			if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE)) {
				RTMPSetTimer(&pAd->Mlme.AutoWakeupTimer,
					     AUTO_WAKEUP_TIMEOUT);
				return;
			}
