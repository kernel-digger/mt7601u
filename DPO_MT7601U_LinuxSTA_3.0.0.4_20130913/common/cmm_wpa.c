...
VOID WpaEAPOLKeyAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	eapol_len =
	    CONV_ARRARY_TO_UINT16(pEapol_packet->Body_Len) + LENGTH_EAPOL_H;
	...
	*((USHORT *) & peerKeyInfo) = cpu2le16(*((USHORT *) & peerKeyInfo));
	...
	do {
		...
		if (!pEntry
		    || (!IS_ENTRY_CLIENT(pEntry) && !IS_ENTRY_APCLI(pEntry)))
			break;
			     ...
			     IN UCHAR MsgType, IN MAC_TABLE_ENTRY * pEntry)
{
	...
	*((USHORT *) & EapolKeyInfo) = cpu2le16(*((USHORT *) & EapolKeyInfo));
	...
	if (bReplayDiff) {
		...
		RTMPSendWirelessEvent(pAd, IW_REPLAY_COUNTER_DIFF_EVENT_FLAG,
	...
	if (MsgType != EAPOL_PAIR_MSG_1) {
		...
		if (EapolKeyInfo.KeyDescVer == KEY_DESC_TKIP) {	/* TKIP */
			RT_HMAC_MD5(pEntry->PTK, LEN_PTK_KCK, (PUCHAR) pMsg,
				    eapol_len, mic, MD5_DIGEST_SIZE);
		} else if (EapolKeyInfo.KeyDescVer == KEY_DESC_AES) {	/* AES        */
			RT_HMAC_SHA1(pEntry->PTK, LEN_PTK_KCK, (PUCHAR) pMsg,
				     eapol_len, digest, SHA1_DIGEST_SIZE);
			NdisMoveMemory(mic, digest, LEN_KEY_DESC_MIC);
		} else if (EapolKeyInfo.KeyDescVer == KEY_DESC_EXT) {	/* AES-128 */
			UINT mlen = AES_KEY128_LENGTH;
			AES_CMAC((PUCHAR) pMsg, eapol_len, pEntry->PTK,
				 LEN_PTK_KCK, mic, &mlen);
		}

		if (!NdisEqualMemory(rcvd_mic, mic, LEN_KEY_DESC_MIC)) {
			/* send wireless event - for MIC different */
			RTMPSendWirelessEvent(pAd, IW_MIC_DIFF_EVENT_FLAG,
					      pEntry->Addr, pEntry->apidx, 0);
		...
		if ((MsgType == EAPOL_PAIR_MSG_3 && bWPA2)
			...
			if ((EapolKeyInfo.KeyDescVer == KEY_DESC_EXT)
				...
				AES_Key_Unwrap(pMsg->KeyDesc.KeyData,
					       ...
					       (pMsg->KeyDesc.KeyDataLen),
					       &pEntry->PTK[LEN_PTK_KCK],
					       LEN_PTK_KEK, KEYDATA,
					       &aes_unwrap_len);
				SET_UINT16_TO_ARRARY(pMsg->KeyDesc.KeyDataLen,
						     aes_unwrap_len);
			} else {
			...
			IN MAC_TABLE_ENTRY * pEntry, IN MLME_QUEUE_ELEM * Elem)
{
	...
	do {
		...
		RTMPToWirelessSta(pAd, pEntry, Header802_3, LENGTH_802_3,
				  (PUCHAR) pEapolFrame,
				  CONV_ARRARY_TO_UINT16(pEapolFrame->Body_Len) +
				  4,
				  (pEntry->PortSecured ==
				   WPA_802_1X_PORT_SECURED) ? FALSE : TRUE);
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		{
			...
			NdisMoveMemory(&pAd->SharedKey[BSS0][0],
				       &pEntry->PairwiseKey,
				       sizeof(CIPHER_KEY));
		}
	}
...
VOID WPAStart2WayGroupHS(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY * pEntry)
{
	...
	ConstructEapolMsg(pEntry,
			  ...
			  default_key,
			  (UCHAR *) gnonce_ptr,
			  TxTsc, (UCHAR *) gtk_ptr, NULL, 0, pEapolFrame);
			    ...
			    IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if ((pEntry) && IS_ENTRY_CLIENT(pEntry)
		...
		case EAPOL_START_PSK:
			...
			MlmeEnqueue(pAd, WPA_STATE_MACHINE, MT2_EAPOLStart, 6,
				    &pEntry->Addr, 0);
			break;
		default:
			break;
	...
	if (pEntry) {
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			MgtMacHeaderInit(pAd, &DeAuthHdr, SUBTYPE_DEAUTH, 0,
					 pEntry->Addr, pAd->CommonCfg.Bssid);
		}
#endif /* CONFIG_STA_SUPPORT */
		...
		if (bDataFrameFirst)
			MiniportMMRequest(pAd, MGMT_USE_QUEUE_FLAG, pOutBuffer,
					  FrameLen);
		else
			 ...
			 IN VOID * Msg, IN UINT MsgLen)
{
	...
	if (MsgLen <
	    (LENGTH_802_1_H + LENGTH_EAPOL_H + MIN_LEN_OF_EAPOL_KEY_MSG))
		return;
	 ...
	 IN UCHAR * data, IN INT data_len, OUT UCHAR * output, IN INT len)
{
	...
	for (i = 0; i < (len + 19) / 20; i++) {
		RT_HMAC_SHA1(key, key_len, input, total_len,
			     &output[currentindex], SHA1_DIGEST_SIZE);
		currentindex += 20;
...
*/
int RtmpPasswordHash(PSTRING password, PUCHAR ssid, INT ssidlength,
		     PUCHAR output)
{
...
VOID KDF(IN PUINT8 key,
	 ...
	 IN PUINT8 label,
	 IN INT label_len,
	 IN PUINT8 data, IN INT data_len, OUT PUINT8 output, IN USHORT len)
{
	USHORT i;
	UCHAR *input;
	INT currentindex = 0;
...
VOID RTMPDerivePMKID(IN PUINT8 pAaddr,
		     IN PUINT8 pSpaddr,
		     IN PUINT8 pKey, IN PUINT8 pAkm_oui, OUT PUINT8 pPMKID)
{
	...
	{
		RT_HMAC_SHA1(pKey, PMK_LEN, text_buf, text_len, digest,
			     SHA1_DIGEST_SIZE);
	}
...
VOID WpaDeriveGTK(IN UCHAR * GMK,
		  IN UCHAR * GNonce,
		...
		case Ndis802_11Encryption4Enabled:
			...
			if (MIX_CIPHER_WPA2_TKIP_ON(FlexibleCipher)) {
				...
				if (MIX_CIPHER_WPA2_AES_ON(FlexibleCipher)) {
					NdisMoveMemory(pRsnIe + sizeof(RSNIE2),
						       OUI_WPA2_CCMP, 4);
					PairwiseCnt = 2;
				}
			} else {
		...
		case Ndis802_11Encryption4Enabled:
			...
			if (MIX_CIPHER_WPA_TKIP_ON(FlexibleCipher)) {
				...
				if (MIX_CIPHER_WPA_AES_ON(FlexibleCipher)) {
					NdisMoveMemory(pRsnIe + sizeof(RSNIE),
						       OUI_WPA_CCMP, 4);
					PairwiseCnt = 2;
				}
			} else {
		   ...
		   IN UINT AuthMode, IN UINT WepStatus, IN UCHAR apidx)
{
	...
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (pAd->ApCfg.ApCliTab[apcliIfidx].WpaSupplicantUP !=
			...
			if (apcliIfidx < MAX_APCLI_NUM) {
				...
				NdisZeroMemory(pAd->ApCfg.
					       ApCliTab[apcliIfidx].RSN_IE,
					       MAX_LEN_OF_RSNIE);
				rsnielen_cur_p =
				...
				bMixCipher =
				    pAd->ApCfg.ApCliTab[apcliIfidx].bMixCipher;
				break;
			} else {
				DBGPRINT(RT_DEBUG_ERROR,
...

	{
		...
		RTMPMakeRsnIeCipher(pAd, PrimaryRsnie, WepStatus, apidx,
				    bMixCipher, FlexibleCipher, pRsnIe,
				    &p_offset);
	...
	if (bWPA2) {
		...
		if ((MsgType == EAPOL_PAIR_MSG_1)
			...
			pMsg->KeyDesc.KeyLength[1] =
			    ((pEntry->WepStatus ==
			      Ndis802_11Encryption2Enabled) ? LEN_TKIP_TK :
			     LEN_AES_TK);
	} else if (!bWPA2) {
		if (MsgType >= EAPOL_GROUP_MSG_1) {
			...
			pMsg->KeyDesc.KeyLength[1] =
			    ((GroupKeyWepStatus ==
			      Ndis802_11Encryption2Enabled) ? LEN_TKIP_GTK :
			     LEN_AES_GTK);
		} else {
			   ...
			   IN UCHAR RSNIE_LEN, OUT PEAPOL_PACKET pMsg)
{
	...
	if ((MsgType == EAPOL_PAIR_MSG_3 && bWPA2Capable)
		...
		if (bWPA2Capable) {
			...
			WPA_ConstructKdeHdr(KDE_GTK, 2 + gtk_len,
...
VOID CalculateMIC(IN UCHAR KeyDescVer, IN UCHAR * PTK, OUT PEAPOL_PACKET pMsg)
{
	...
	/* Calculate MIC */
	if (KeyDescVer == KEY_DESC_AES) {
		RT_HMAC_SHA1(PTK, LEN_PTK_KCK, OutBuffer, FrameLen, digest,
			     SHA1_DIGEST_SIZE);
		NdisMoveMemory(mic, digest, LEN_KEY_DESC_MIC);
	} else if (KeyDescVer == KEY_DESC_TKIP) {
		RT_HMAC_MD5(PTK, LEN_PTK_KCK, OutBuffer, FrameLen, mic,
			    MD5_DIGEST_SIZE);
...
PCIPHER_KEY RTMPSwCipherKeySelection(IN RTMP_ADAPTER * pAd,
				     IN PUCHAR pIV,
				     IN RX_BLK * pRxBlk,
				     IN MAC_TABLE_ENTRY * pEntry)
{
				     ...
				     INOUT UINT16 * DataByteCnt)
{
	...
	case CIPHER_WEP128:
		...
		if (RTMPSoftDecryptWEP(pAd, pKey, pData, &(*DataByteCnt)) ==
		    FALSE) {
			DBGPRINT(RT_DEBUG_ERROR,
	...
	case CIPHER_AES:
		...
		if (RTMPSoftDecryptCCMP(pAd, pHdr, pKey, pData, &(*DataByteCnt))
		    == FALSE) {
			DBGPRINT(RT_DEBUG_ERROR,
...
VOID RTMPSoftConstructIVHdr(IN UCHAR CipherAlg,
			    IN UCHAR key_id,
			    IN PUCHAR pTxIv,
			    OUT PUCHAR pHdrIv, OUT UINT8 * hdr_iv_len)
{
	*hdr_iv_len = 0;
...
VOID RTMPSoftEncryptionAction(IN PRTMP_ADAPTER pAd,
			      ...
			      IN UINT32 SrcBufLen,
			      IN UCHAR KeyIdx,
			      IN PCIPHER_KEY pKey, OUT UINT8 * ext_len)
{
	*ext_len = 0;
	...
	if ((CipherAlg == CIPHER_WEP64) || (CipherAlg == CIPHER_WEP128)) {
		...
		RTMPSoftEncryptWEP(pAd,
				   pKey->TxTsc, pKey, pSrcBufData, SrcBufLen);
		...
		*ext_len = LEN_ICV;
		...
		RTMPSoftEncryptCCMP(pAd,
				    pHdr,
				    pKey->TxTsc,
				    pKey->Key, pSrcBufData, SrcBufLen);
	...
	*count = 0;
	...
	if (len < sizeof(UINT16)) {
		...
		if (p_count > 0) {
			...
			offset =
			    sizeof(RSNIE_PMKID) + (LEN_PMKID * (p_count - 1));
...
VOID WpaShowAllsuite(IN PUINT8 rsnie, IN UINT rsnie_len)
{
	...
	/* group cipher */
	if ((pSuite =
	     WPA_ExtractSuiteFromRSNIE(rsnie, rsnie_len, GROUP_SUITE,
				       &count)) != NULL) {
	...
	/* pairwise cipher */
	if ((pSuite =
	     WPA_ExtractSuiteFromRSNIE(rsnie, rsnie_len, PAIRWISE_SUITE,
				       &count)) != NULL) {
	...
	/* AKM */
	if ((pSuite =
	     WPA_ExtractSuiteFromRSNIE(rsnie, rsnie_len, AKM_SUITE,
				       &count)) != NULL) {
	...
	/* PMKID */
	if ((pSuite =
	     WPA_ExtractSuiteFromRSNIE(rsnie, rsnie_len, PMKID_LIST,
				       &count)) != NULL) {
...
VOID WPAInstallPairwiseKey(PRTMP_ADAPTER pAd,
			   UINT8 BssIdx, PMAC_TABLE_ENTRY pEntry, BOOLEAN bAE)
{
	...
	NdisMoveMemory(pEntry->PairwiseKey.Key, &pEntry->PTK[OFFSET_OF_PTK_TK],
		       LEN_TK);
	if (pEntry->PairwiseKey.CipherAlg == CIPHER_TKIP) {
		if (bAE) {
			NdisMoveMemory(pEntry->PairwiseKey.TxMic,
				       &pEntry->PTK[OFFSET_OF_AP_TKIP_TX_MIC],
				       LEN_TKIP_MIC);
			NdisMoveMemory(pEntry->PairwiseKey.RxMic,
				       &pEntry->PTK[OFFSET_OF_AP_TKIP_RX_MIC],
				       LEN_TKIP_MIC);
		} else {
			NdisMoveMemory(pEntry->PairwiseKey.TxMic,
				       &pEntry->PTK[OFFSET_OF_STA_TKIP_TX_MIC],
				       LEN_TKIP_MIC);
			NdisMoveMemory(pEntry->PairwiseKey.RxMic,
				       &pEntry->PTK[OFFSET_OF_STA_TKIP_RX_MIC],
				       LEN_TKIP_MIC);
		}
	}
#ifdef SOFT_ENCRYPT
...
#endif /* SOFT_ENCRYPT */
	{
		...
		RTMPSetWcidSecurityInfo(pAd,
					...
					(UCHAR) pEntry->Aid, PAIRWISEKEYTABLE);
	}
...
VOID WPAInstallSharedKey(PRTMP_ADAPTER pAd,
			 ...
			 UINT8 BssIdx,
			 UINT8 KeyIdx,
			 UINT8 Wcid, BOOLEAN bAE, PUINT8 pGtk, UINT8 GtkLen)
{
	...
	if (GroupCipher == Ndis802_11GroupWEP40Enabled ||
		...
		if (pSharedKey->CipherAlg == CIPHER_TKIP) {
			if (bAE) {
				NdisMoveMemory(pSharedKey->TxMic, pGtk + 16,
					       LEN_TKIP_MIC);
				NdisMoveMemory(pSharedKey->RxMic, pGtk + 24,
					       LEN_TKIP_MIC);
			} else {
				NdisMoveMemory(pSharedKey->TxMic, pGtk + 24,
					       LEN_TKIP_MIC);
				NdisMoveMemory(pSharedKey->RxMic, pGtk + 16,
					       LEN_TKIP_MIC);
			}
		}
	}
			     ...
			     UINT8 CipherAlg, UINT8 Wcid, UINT8 KeyTabFlag)
{
	...
	AsicUpdateWcidAttributeEntry(pAd,
				     BssIdx,
				     KeyIdx, CipherAlg, Wcid, KeyTabFlag);
