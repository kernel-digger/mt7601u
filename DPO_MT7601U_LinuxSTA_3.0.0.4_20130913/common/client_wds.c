...
PCLIWDS_PROXY_ENTRY CliWdsEntyAlloc(IN PRTMP_ADAPTER pAd)
{
	...
	pCliWdsEntry =
	    (PCLIWDS_PROXY_ENTRY) removeHeadList(&pAd->
						 ApCfg.CliWdsEntryFreeList);
	...
	while (pCliWdsEntry) {
		if (MAC_ADDR_EQUAL(pMac, pCliWdsEntry->Addr)) {
			...
			if (VALID_WCID(pCliWdsEntry->Aid))
				return pAd->MacTab.Content[pCliWdsEntry->
							   Aid].Addr;
			else
				return NULL;
		}
...
VOID CliWds_ProxyTabUpdate(IN PRTMP_ADAPTER pAd, IN SHORT Aid, IN PUCHAR pMac)
{
	...
	if (pCliWdsEntry) {
		...
		insertTailList(&pAd->ApCfg.CliWdsProxyTab[HashId],
			       (PLIST_ENTRY) pCliWdsEntry);
	}
	return;
}
...
VOID CliWds_ProxyTabMaintain(IN PRTMP_ADAPTER pAd)
{
	...
	for (idx = 0; idx < CLIWDS_HASH_TAB_SIZE; idx++) {
		pCliWdsEntry =
		    (PCLIWDS_PROXY_ENTRY) (pAd->ApCfg.
					   CliWdsProxyTab[idx].pHead);
		while (pCliWdsEntry) {
			PCLIWDS_PROXY_ENTRY pCliWdsEntryNext =
			    pCliWdsEntry->pNext;
			if (RTMP_TIME_AFTER
			    (Now,
			     ...
			     (CLI_WDS_ENTRY_AGEOUT * OS_HZ / 1000))) {
				delEntryList(&pAd->ApCfg.CliWdsProxyTab[idx],
					     (PLIST_ENTRY) pCliWdsEntry);
				CliWdsEntyFree(pAd, pCliWdsEntry);
			}
			pCliWdsEntry = pCliWdsEntryNext;
