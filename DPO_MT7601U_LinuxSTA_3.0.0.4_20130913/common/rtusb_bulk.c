...
UCHAR EpToQueue[6] =
    { FIFO_EDCA, FIFO_EDCA, FIFO_EDCA, FIFO_EDCA, FIFO_HCCA, FIFO_MGMT };
#else
UCHAR EpToQueue[6] =
...
VOID SoftwareFlowControl(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < NUM_OF_TX_RING; i++) {
		...
		if (pAd->TxContext[i].CurWritePosition >=
		...
		if (pAd->BulkOutDataSizeCount[i] > 20480
		    || PacketsInQueueSize > 6144) {
			RunningQueueNo++;
	...
	if (ResetBulkOutSize == TRUE) {
		for (QueIdx = 0; QueIdx < NUM_OF_TX_RING; QueIdx++) {
			...
			pAd->BulkOutDataSizeLimit[QueIdx] =
			    MaxBulkOutsSizeLimit[RunningQueueNo]
			    [HighWorkingAcCount];
		}
		     ...
		     IN UCHAR BulkOutPipeId, IN usb_complete_t Func)
{
	...
	if (pTxContext->bAggregatible) {
		...
		RTUSB_FILL_TX_BULK_URB(pUrb,
				       pObj->pUsb_Dev,
				       pChipCap->WMM0ACBulkOutAddr
				       [BulkOutPipeId], pSrc,
				       pTxContext->BulkOutSize, Func,
				       pTxContext,
				       (pTxContext->data_dma +
					TX_BUFFER_NORMSIZE + 2));
	} else {
		pSrc =
		       ...
		       IN ULONG BulkOutSize, IN usb_complete_t Func)
{
	...
	pSrc =
	    &pTxContext->TransferBuffer->field.
	    WirelessPacket[pTxContext->NextBulkOutPosition];
	...
	if (BulkOutPipeId == 4)	//Use EP9
	{
		...
		RTUSB_FILL_HTTX_BULK_URB(pUrb,
					 pObj->pUsb_Dev,
					 pChipCap->WMM0ACBulkOutAddr
					 [BulkOutPipeId], pSrc, BulkOutSize,
					 Func, pTxContext,
					 (pTxContext->data_dma +
					  pTxContext->NextBulkOutPosition));
	}
...
VOID RTUSBInitCmdRspEventDesc(PRTMP_ADAPTER pAd,
			    ...
			    IN UCHAR BulkOutPipeId, IN UCHAR Index)
{
	...
	do {
		...
		pTxWI =
		    (TXWI_STRUC *) & pWirelessPkt[TmpBulkEndPos + TXINFO_SIZE];
...
#ifdef RT65xx
			if (((ThisBulkSize & 0xfffe0000) != 0)
			    || ((ThisBulkSize & 0x10000) == 0x10000))
#else
			if (((ThisBulkSize & 0xffff8000) != 0)
			    || ((ThisBulkSize & 0x6000) == 0x6000))
#endif /* RT65xx */
...
#ifdef RT_BIG_ENDIAN
			RTMPDescriptorEndianChange((PUCHAR) pTxInfo,
						   TYPE_TXINFO);
					     ...
					     purbb_t pURB, pregs * pt_regs)
{
	...
	case EDCA_AC0_PIPE:
		...
		if (!ATE_ON(pAd)) {
#endif /* RALINK_ATE */
			RTMP_NET_TASK_DATA_ASSIGN(&pObj->ac0_dma_done_task,
						  (unsigned long)pURB);
		...
		break;
	case EDCA_AC1_PIPE:
		RTMP_NET_TASK_DATA_ASSIGN(&pObj->ac1_dma_done_task,
					  (unsigned long)pURB);
		RTMP_OS_TASKLET_SCHE(&pObj->ac1_dma_done_task);
		break;
	case EDCA_AC2_PIPE:
		RTMP_NET_TASK_DATA_ASSIGN(&pObj->ac2_dma_done_task,
					  (unsigned long)pURB);
		RTMP_OS_TASKLET_SCHE(&pObj->ac2_dma_done_task);
		break;
	case EDCA_AC3_PIPE:
		RTMP_NET_TASK_DATA_ASSIGN(&pObj->ac3_dma_done_task,
					  (unsigned long)pURB);
		RTMP_OS_TASKLET_SCHE(&pObj->ac3_dma_done_task);
		break;
	case HCCA_PIPE:
		RTMP_NET_TASK_DATA_ASSIGN(&pObj->hcca_dma_done_task,
					  (unsigned long)pURB);
...
#ifdef RT_BIG_ENDIAN
	RTMPDescriptorEndianChange((PUCHAR) pNullContext->TransferBuffer,
				   TYPE_TXINFO);
#endif /* RT_BIG_ENDIAN */
					    ...
					    purbb_t pURB, pregs * pt_regs)
{
	...
	pObj = (POS_COOKIE) pAd->OS_Cookie;
	RTMP_NET_TASK_DATA_ASSIGN(&pObj->null_frame_complete_task,
				  (unsigned long)pURB);
						...
						purbb_t pURB, pregs * pt_regs)
{
	...
	pObj = (POS_COOKIE) pAd->OS_Cookie;
	RTMP_NET_TASK_DATA_ASSIGN(&pObj->hcca_null_frame_complete_task,
				  (unsigned long)pURB);
...
#ifdef RT_BIG_ENDIAN
	RTMPDescriptorEndianChange((PUCHAR) pMLMEContext->TransferBuffer,
				   TYPE_TXINFO);
#endif /* RT_BIG_ENDIAN */
					     ...
					     purbb_t pURB, pregs * pt_regs)
{
	...
	RTMP_NET_TASK_DATA_ASSIGN(&pObj->mgmt_dma_done_task,
				  (unsigned long)pURB);
...
VOID RTUSBBulkOutPsPoll(IN PRTMP_ADAPTER pAd)
{
...
#ifdef RT_BIG_ENDIAN
	RTMPDescriptorEndianChange((PUCHAR) pPsPollContext->TransferBuffer,
				   TYPE_TXINFO);
#endif /* RT_BIG_ENDIAN */
	...
	if (pAd->Multi_Channel_Enable == TRUE)
		RTUSBInitTxDesc(pAd, pPsPollContext, 0,
					 ...
					 pregs * pt_regs)
{
	...
	pObj = (POS_COOKIE) pAd->OS_Cookie;
	RTMP_NET_TASK_DATA_ASSIGN(&pObj->pspoll_frame_complete_task,
				  (unsigned long)pURB);
...
VOID BulkInCmdRspEvent(RTMP_ADAPTER * pAd)
{
	...
	if ((pCmdRspEventContext->Readable == TRUE)
...
VOID RTUSBBulkCmdRspEventReceive(PRTMP_ADAPTER pAd)
{
	...
	RTMP_IRQ_LOCK(&pAd->CmdRspLock, IrqFlags);
	if ((pCmdRspEventContext->InUse) == FALSE
					   ...
					   purbb_t pURB, pregs * pt_regs)
{
	...
	RTMP_NET_TASK_DATA_ASSIGN(&pObj->cmd_rsp_event_task,
				  (unsigned long)pURB);
	...
	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NEED_STOP_TX)
		...
		if (RTUSB_TEST_BULK_FLAG(pAd, fRTUSB_BULK_OUT_DATA_NORMAL)) {
			if (((!RTMP_TEST_FLAG
				...
				RTUSBBulkOutDataPacket(pAd, EDCA_AC0_PIPE,
						       pAd->NextBulkOutIndex
						       [EDCA_AC0_PIPE]);
			}
		}
		if (RTUSB_TEST_BULK_FLAG(pAd, fRTUSB_BULK_OUT_DATA_NORMAL_2)) {
			if (((!RTMP_TEST_FLAG
				...
				RTUSBBulkOutDataPacket(pAd, EDCA_AC1_PIPE,
						       pAd->NextBulkOutIndex
						       [EDCA_AC1_PIPE]);
			}
		}
		if (RTUSB_TEST_BULK_FLAG(pAd, fRTUSB_BULK_OUT_DATA_NORMAL_3)) {
			if (((!RTMP_TEST_FLAG
				...
				RTUSBBulkOutDataPacket(pAd, EDCA_AC2_PIPE,
						       pAd->NextBulkOutIndex
						       [EDCA_AC2_PIPE]);
			}
		}
		if (RTUSB_TEST_BULK_FLAG(pAd, fRTUSB_BULK_OUT_DATA_NORMAL_4)) {
			if (((!RTMP_TEST_FLAG
				...
				RTUSBBulkOutDataPacket(pAd, EDCA_AC3_PIPE,
						       pAd->NextBulkOutIndex
						       [EDCA_AC3_PIPE]);
			}
		}
		...
		if (RTUSB_TEST_BULK_FLAG(pAd, fRTUSB_BULK_OUT_DATA_HCCA)) {
			if (((!RTMP_TEST_FLAG
				...
				RTUSBBulkOutDataPacket(pAd, HCCA_PIPE,
						       pAd->NextBulkOutIndex
						       [HCCA_PIPE]);
			}
		}
...
#ifdef CONFIG_MULTI_CHANNEL
		else if (RTUSB_TEST_BULK_FLAG
			 (pAd, fRTUSB_BULK_OUT_DATA_NULL_HCCA)) {
			if (INFRA_ON(pAd)
			    && (pAd->CommonCfg.Channel ==
				pAd->LatchRfRegs.Channel
				|| pAd->CommonCfg.CentralChannel ==
