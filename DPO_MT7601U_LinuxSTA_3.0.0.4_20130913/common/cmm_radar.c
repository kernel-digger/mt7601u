...
INT Set_RadarShow_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (i = 0; i < pRadarDetect->fdf_num; i++) {
		printk("ChBusyThrd[%d] = %d\n", i,
		       pRadarDetect->ch_busy_threshold[i]);
		printk("RssiThrd[%d] = %d\n", i,
		       pRadarDetect->rssi_threshold[i]);
	}
	...
	pDfsSwParam->TotalEntries[0] = pDfsSwParam->TotalEntries[1] =
	    pDfsSwParam->TotalEntries[2] = pDfsSwParam->TotalEntries[3] = 0;
