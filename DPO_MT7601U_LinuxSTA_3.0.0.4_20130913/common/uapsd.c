...
========================================================================
*/
VOID UAPSD_Init(IN PRTMP_ADAPTER pAd)
{
...
========================================================================
*/
VOID UAPSD_Release(IN PRTMP_ADAPTER pAd)
{
...
========================================================================
*/
VOID RtmpAsicSleepHandle(IN PRTMP_ADAPTER pAd)
{
...
========================================================================
*/
VOID UAPSD_SP_Close(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY * pEntry)
{
...
========================================================================
*/
BOOLEAN UAPSD_SP_IsClosed(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY * pEntry)
{
...
========================================================================
*/
VOID UAPSD_AllPacketDeliver(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (pEntry->pUAPSDEOSPFrame != NULL) {
		...
		QueId =
		    RTMP_GET_PACKET_UAPSD_QUE_ID(QUEUE_ENTRY_TO_PACKET
						 (pEntry->pUAPSDEOSPFrame));
...
========================================================================
*/
VOID UAPSD_AssocParse(IN PRTMP_ADAPTER pAd,
		      IN MAC_TABLE_ENTRY * pEntry,
		      IN UCHAR * pElm, IN BOOLEAN FlgApsdCapable)
{
	...
	if ((pEntry) && (FlgApsdCapable == TRUE)) {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("apsd> UAPSD %d %d %d %d!\n",
					  pQosInfo->UAPSD_AC_BE,
					  pQosInfo->UAPSD_AC_BK,
					  pQosInfo->UAPSD_AC_VI,
					  pQosInfo->UAPSD_AC_VO));
		DBGPRINT(RT_DEBUG_TRACE,
			 ("apsd> MaxSPLength = %d\n", pEntry->MaxSPLength));
		...
		for (IdApsd = 0; IdApsd < 4; IdApsd++) {
			...
			pEntry->bAPSDDeliverEnabledPerAC[IdApsd] =
			    UAPSD[IdApsd];
	...
	if (pQueUapsd->Number >= MAX_PACKETS_IN_UAPSD_QUEUE) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("uapsd> many(%ld) WCID(%d) AC(%d)\n",
			  pQueUapsd->Number,
			  RTMP_GET_PACKET_WCID(pPacket), IdAc));
...
========================================================================
*/
VOID UAPSD_QueueMaintenance(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY * pEntry)
{
	QUEUE_HEADER *pQue;
	UINT32 IdAc;
...
========================================================================
*/
VOID UAPSD_SP_AUE_Handle(IN RTMP_ADAPTER * pAd,
...
#ifdef UAPSD_DEBUG
		DBGPRINT(RT_DEBUG_TRACE,
			 ("uapsd> aux: Station actives! Close SP!\n"));
#endif /* UAPSD_DEBUG */
		//pEntry->bAPSDFlagSPStart = 0;
		pEntry->bAPSDFlagEOSPOK = 0;
		UAPSD_SP_END(pAd, pEntry);
...
#ifdef UAPSD_DEBUG
			DBGPRINT(RT_DEBUG_TRACE,
				 ("uapsd> legacy PS packet is sent!\n"));
#endif /* UAPSD_DEBUG */
		} else {
#ifdef UAPSD_TIMING_RECORD_FUNC
			...
			if (pEntry->UAPSDTxNum == 1) {
				...
				if (pEntry->pUAPSDEOSPFrame != NULL) {
					...
					pPkt =
					    ...
					    (pEntry->pUAPSDEOSPFrame);
					QueId =
					    RTMP_GET_PACKET_UAPSD_QUE_ID(pPkt);
...
========================================================================
*/
VOID UAPSD_SP_CloseInRVDone(IN PRTMP_ADAPTER pAd)
{
	UINT32 IdEntry;
	int FirstWcid = 0;
...
#ifdef UAPSD_DEBUG
			DBGPRINT(RT_DEBUG_TRACE,
				 ("uapsd> close SP in UAPSD_SP_CloseInRVDone()!\n\n\n"));
#endif /* UAPSD_DEBUG */
...
========================================================================
*/
VOID UAPSD_TimingRecordCtrl(IN UINT32 Flag)
{
...
========================================================================
*/
VOID UAPSD_TimingRecord(IN PRTMP_ADAPTER pAd, IN UINT32 Type)
{
	UINT32 Index;
	...
	case UAPSD_TIMING_RECORD_TX2AIR:
		...
		gUAPSD_TimingSumIsr2Tasklet +=
		    (UINT32) (gUAPSD_TimingTasklet[Index] -
			      gUAPSD_TimingIsr[Index]);
		gUAPSD_TimingSumTrig2Txqueue +=
		    (UINT32) (gUAPSD_TimingMov2Tx[Index] -
			      gUAPSD_TimingTrgRcv[Index]);
		gUAPSD_TimingSumTxqueue2Air +=
		    (UINT32) (gUAPSD_TimingTx2Air[Index] -
			      gUAPSD_TimingMov2Tx[Index]);
...
========================================================================
*/
VOID UAPSD_TimeingRecordLoopIndex(IN UINT32 LoopIndex)
{
...
========================================================================
*/
BOOLEAN UAPSD_PsPollHandle(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY * pEntry)
{
...
========================================================================
*/
VOID UAPSD_TriggerFrameHandle(IN PRTMP_ADAPTER pAd,
...
#ifdef UAPSD_DEBUG
		DBGPRINT(RT_DEBUG_TRACE,
			 ("uapsd> sorry! SP is not yet closed!\n"));
#endif /* UAPSD_DEBUG */
...
/*	RTMP_IRQ_LOCK(FlgIrq); */
	...
	for (IdAc = (WMM_NUM_OF_AC - 1); IdAc >= 0; IdAc--) {
		...
		while (pAcPsQue->Head) {
			...
			if (pQuedPkt != NULL) {
				 ...
				 */
				UAPSD_INSERT_QUEUE_AC(pAd, pEntry, pLastAcSwQue,
						      pQuedPkt);
			}
...
#ifdef UAPSD_DEBUG
	{
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("uapsd> start a SP (Tx Num = %d) (Rough SP = %d) "
	...
	if ((pEntry->pUAPSDEOSPFrame != NULL) &&
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("uapsd> The EOSP frame is a management frame.\n"));
	}
#endif /* UAPSD_DEBUG */
...
#ifdef UAPSD_DEBUG
		DBGPRINT(RT_DEBUG_TRACE,
			 ("uapsd> A legacy PS is sent! UAPSDTxNum = %d\n",
			  pEntry->UAPSDTxNum));
#endif /* UAPSD_DEBUG */
	}
#endif /* UAPSD_SP_ACCURATE */
...
#ifdef UAPSD_DEBUG
		DBGPRINT(RT_DEBUG_TRACE,
			 ("uapsd> end a SP by a QoS Null frame!\n"));
#endif /* UAPSD_DEBUG */
	}
#ifdef UAPSD_TIMING_RECORD_FUNC
