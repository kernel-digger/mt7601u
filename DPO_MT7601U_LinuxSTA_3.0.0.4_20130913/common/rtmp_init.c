...
#ifdef DBG
char *CipherName[] =
    { "none", "wep64", "wep128", "TKIP", "AES", "CKIP64", "CKIP128", "CKIP152",
	"SMS4"
};
#endif
...
/*		pBeaconBuf = kmalloc(MAX_BEACON_SIZE, MEM_ALLOC_FLAG);*/
		...
		/* 分配RTMP_ADAPTER空间 */
		Status =
		    AdapterBlockAllocateMemory(handle, (PVOID *) & pAd,
					       sizeof(RTMP_ADAPTER));
...
#ifdef RALINK_ATE
#ifdef RTMP_MAC_USB
		RTMP_OS_ATMOIC_INIT(&pAd->BulkOutRemained,
				    &pAd->RscAtomicMemList);
		RTMP_OS_ATMOIC_INIT(&pAd->BulkInRemained,
				    &pAd->RscAtomicMemList);
#endif /* RTMP_MAC_USB */
#endif /* RALINK_ATE */
...
/*		kfree(pBeaconBuf);*/
	...
	for (index = 0; index < MAX_LEN_OF_BSS_TABLE; index++) {
		if (os_alloc_mem(pAd, &pAd->ProbeRespIE[index].pIe, MAX_VIE_LEN)
		    == NDIS_STATUS_SUCCESS)
			RTMPZeroMemory(pAd->ProbeRespIE[index].pIe,
				       MAX_VIE_LEN);
		else
...
VOID NICReadEEPROMParameters(RTMP_ADAPTER * pAd, PSTRING mac_addr)
{
	...
	if (pAd->chipOps.eeinit) {
		...
		if (!pAd->bFroceEEPROMBuffer && pAd->bEEPROMFile) {
			DBGPRINT(RT_DEBUG_OFF,
				 ("--> NICReadEEPROMParameters::(Efuse)Load to EEPROM Buffer Mode\n"));
			eFuseLoadEEPROM(pAd);
		}
#endif /* RALINK_ATE */
...
// TODO: check this after receive TC6008 EEPROM format
	...
	if (IS_RT8592(pAd)) {
		DBGPRINT(RT_DEBUG_OFF,
			 ("RT85592: Antenna.RfIcType=%d, TxPath=%d, RxPath=%d\n",
			  Antenna.field.RfIcType, Antenna.field.TxPath,
	...
	if (((Antenna.word & 0xFF00) != 0xFF00) && (Antenna.word & 0x2000)) {
		...
		DBGPRINT(RT_DEBUG_OFF,
			 ("\x1b[mAntenna word %X/%d, AntDiv %d\x1b[m\n",
...
/*	if (Antenna.word == 0xFFFF)*/
		...
		/* 0. 11b/g */
	{
		...
		{
			RT28xx_EEPROM_READ16(pAd, EEPROM_G_TSSI_BOUND1,
					     Power.word);
			pAd->TssiMinusBoundaryG[4] = Power.field.Byte0;
			pAd->TssiMinusBoundaryG[3] = Power.field.Byte1;
			RT28xx_EEPROM_READ16(pAd, EEPROM_G_TSSI_BOUND2,
					     Power.word);
			pAd->TssiMinusBoundaryG[2] = Power.field.Byte0;
			pAd->TssiMinusBoundaryG[1] = Power.field.Byte1;
			RT28xx_EEPROM_READ16(pAd, EEPROM_G_TSSI_BOUND3,
					     Power.word);
			pAd->TssiRefG = Power.field.Byte0;	/* reference value [0] */
			pAd->TssiPlusBoundaryG[1] = Power.field.Byte1;
			RT28xx_EEPROM_READ16(pAd, EEPROM_G_TSSI_BOUND4,
					     Power.word);
			pAd->TssiPlusBoundaryG[2] = Power.field.Byte0;
			pAd->TssiPlusBoundaryG[3] = Power.field.Byte1;
			RT28xx_EEPROM_READ16(pAd, EEPROM_G_TSSI_BOUND5,
					     Power.word);
	...
	{
		{
			RT28xx_EEPROM_READ16(pAd, EEPROM_A_TSSI_BOUND1,
					     Power.word);
			pAd->TssiMinusBoundaryA[4] = Power.field.Byte0;
			pAd->TssiMinusBoundaryA[3] = Power.field.Byte1;
			RT28xx_EEPROM_READ16(pAd, EEPROM_A_TSSI_BOUND2,
					     Power.word);
			pAd->TssiMinusBoundaryA[2] = Power.field.Byte0;
			pAd->TssiMinusBoundaryA[1] = Power.field.Byte1;
			RT28xx_EEPROM_READ16(pAd, EEPROM_A_TSSI_BOUND3,
					     Power.word);
			pAd->TssiRefA = Power.field.Byte0;
			pAd->TssiPlusBoundaryA[1] = Power.field.Byte1;
			RT28xx_EEPROM_READ16(pAd, EEPROM_A_TSSI_BOUND4,
					     Power.word);
			pAd->TssiPlusBoundaryA[2] = Power.field.Byte0;
			pAd->TssiPlusBoundaryA[3] = Power.field.Byte1;
			RT28xx_EEPROM_READ16(pAd, EEPROM_A_TSSI_BOUND5,
					     Power.word);
...
#ifdef MT7601
	if (IS_MT7601(pAd)) {
		RT28xx_EEPROM_READ16(pAd, EEPROM_FREQ_OFFSET_COMPERSATION,
				     value);
		value = (value >> 8) & 0xFF;
		if (value != 0xFF) {
			if (value & 0x80)
...
/*		if (IS_RT2860(pAd))  RT2860 supports 3 Rx and the 2.4 GHz RSSI #2 offset is in the EEPROM 0x48*/
...
#ifdef SINGLE_SKU
	{
		RT28xx_EEPROM_READ16(pAd, EEPROM_DEFINE_MAX_TXPWR,
	...
	if ((pAd->CommonCfg.DefineMaxTxPwr & 0xFF) <= 0x50) {
		...
		else if ((pAd->CommonCfg.AntGain > 0)
			 && (pAd->CommonCfg.BandedgeDelta >= 0))
			pAd->CommonCfg.bSKUMode = TRUE;
	}
	DBGPRINT(RT_DEBUG_TRACE, ("Single SKU Mode is %s\n",
				  pAd->CommonCfg.
				  bSKUMode ? "Enable" : "Disable"));
#endif /* SINGLE_SKU */
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s: pAd->Antenna.field.BoardType = %d, IS_MINI_CARD(pAd) = %d, IS_RT5390U(pAd) = %d\n",
		  __FUNCTION__, pAd->Antenna.field.BoardType, IS_MINI_CARD(pAd),
		  IS_RT5390U(pAd)));
...
*/
	...
	DBGPRINT(RT_DEBUG_TRACE, ("TxPath = %d, RxPath = %d, RFIC=%d\n",
				  ...
				  pAd->Antenna.field.RxPath, pAd->RfIcType));
	DBGPRINT(RT_DEBUG_TRACE, ("<-- NICInitAsicFromEEPROM\n"));
}
...
VOID AsicInitBcnBuf(IN RTMP_ADAPTER * pAd)
{
	...
	for (idx = 0; idx < pChipCap->BcnMaxHwNum; idx++) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("\t\tBcnBase[%d] = 0x%x, pAd->BeaconOffset[%d]=0x%x\n",
...
#ifdef RLT_MAC
	{
		...
		for (idx = 0; idx < 4; idx++) {
			RTMP_IO_WRITE32(pAd,
					(USHORT) bcn_mac_reg_tb[idx].Register,
...
NDIS_STATUS NICInitializeAsic(IN PRTMP_ADAPTER pAd, IN BOOLEAN bHardReset)
{
	...
	if ((IS_RT3883(pAd)) || IS_RT65XX(pAd) || IS_MT7601(pAd) || ((pAd->MACVersion >= RALINK_2880E_VERSION) && (pAd->MACVersion < RALINK_3070_VERSION))) {	/* 3*3 */
		/* enlarge MAX_LEN_CFG */
		UINT32 csr;
...
/*			RTMP_IO_WRITE32(pAd, PWR_PIN_CFG, 0x00001818);*/
	...
	if (bHardReset) {
		...
		/* Clear all pairwise key table when initial */
		{
			...
			AndesBurstWrite(pAd, MAC_WCID_ATTRIBUTE_BASE, MACValue,
					256);
		}
#else
			 ...
			 IN TX_STA_CNT1_STRUC * pStaTxCnt1)
{
	...
	pAd->RalinkCounters.OneSecBeaconSentCnt +=
	    pStaTxCnt0->field.TxBeaconCount;
	pAd->RalinkCounters.OneSecTxRetryOkCount +=
	    pStaTxCnt1->field.TxRetransmit;
...
VOID NICUpdateRawCounters(IN PRTMP_ADAPTER pAd)
{
...

	{
		...
		pAd->RalinkCounters.FalseCCACnt +=
...
#ifdef DBG_DIAGNOSE
	{
		...
		if (pDiag->inited == 0) {
			...
			/* Tx */
			pDiag->TxFailCnt[ArrayCurIdx] =
			    TxStaCnt0.field.TxFailCount;
			pDiag->TxAggCnt[ArrayCurIdx] =
			    TxAggCnt.field.AggTxCount;
			pDiag->TxNonAggCnt[ArrayCurIdx] =
			    TxAggCnt.field.NonAggTxCount;

			pDiag->TxAMPDUCnt[ArrayCurIdx][0] =
			    TxAggCnt0.field.AggSize1Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][1] =
			    TxAggCnt0.field.AggSize2Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][2] =
			    TxAggCnt1.field.AggSize3Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][3] =
			    TxAggCnt1.field.AggSize4Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][4] =
			    TxAggCnt2.field.AggSize5Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][5] =
			    TxAggCnt2.field.AggSize6Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][6] =
			    TxAggCnt3.field.AggSize7Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][7] =
			    TxAggCnt3.field.AggSize8Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][8] =
			    TxAggCnt4.field.AggSize9Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][9] =
			    TxAggCnt4.field.AggSize10Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][10] =
			    TxAggCnt5.field.AggSize11Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][11] =
			    TxAggCnt5.field.AggSize12Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][12] =
			    TxAggCnt6.field.AggSize13Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][13] =
			    TxAggCnt6.field.AggSize14Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][14] =
			    TxAggCnt7.field.AggSize15Count;
			pDiag->TxAMPDUCnt[ArrayCurIdx][15] =
			    TxAggCnt7.field.AggSize16Count;

			pDiag->RxCrcErrCnt[ArrayCurIdx] =
			    RxStaCnt0.field.CrcErr;
...
/*			for (i = 9; i < 16; i++)*/
			...
			if (pDiag->ArrayCurIdx == pDiag->ArrayStartIdx)
				INC_RING_INDEX(pDiag->ArrayStartIdx,
					       DIAGNOSE_TIME);
		}
...
/*	EDCA_PARM DefaultEdcaParm;*/
	...
	for (key_index = 0; key_index < SHARE_KEY_NUM; key_index++) {
		for (bss_index = 0;
		     bss_index < MAX_MBSSID_NUM(pAd) + MAX_P2P_NUM;
		     bss_index++) {
...
BOOLEAN PairEP(RTMP_ADAPTER * pAd, UINT8 EP, UINT8 Index, UINT8 InOut)
{
	...
	if (Index == 0 && InOut == 0) {
		...
		DBGPRINT(RT_DEBUG_OFF,
			 ("Endpoint(%x) is for In-band Command\n", EP));
		return TRUE;
	}
...
#ifdef RTMP_MAC_USB
#if 0
	Endpoint(8) is for In - band Command Endpoint(4)
	is for WMM0 AC0 Endpoint(5)
	is for WMM0 AC1 Endpoint(6)
	is for WMM0 AC2 Endpoint(7)
	is for WMM0 AC3 Endpoint(9)
	is for WMM1 AC0 Endpoint(84)
	is for Data - In Endpoint(85)
		is for Command Rsp
#endif
	for (i = 0; i < 6; i++) {
...
VOID RTMP_BBP_IO_WRITE8_BY_REG_ID(PRTMP_ADAPTER pAd, UINT32 Offset, UINT8 Value)
{
#ifdef RT8592
	if (IS_RT8592(pAd))
		return;
...
VOID AntCfgInit(IN PRTMP_ADAPTER pAd)
{
...

	{
		if (pAd->NicConfig2.field.AntOpt == 1) {	/* ant selected by efuse */
			...
			pAd->RxAnt.Pair1PrimaryRxAnt = 0;
			pAd->RxAnt.Pair1SecondaryRxAnt = 1;
		} else if (pAd->NicConfig2.field.AntDiversity == 1) {	/* Ant div on *//* eeprom on, but sw ant div support is not enabled: default ant is main */
			pAd->RxAnt.Pair1PrimaryRxAnt = 0;
			pAd->RxAnt.Pair1SecondaryRxAnt = 1;
		}
	}
...
VOID NICUpdateRxStatusCnt1(IN PRTMP_ADAPTER pAd, IN UCHAR Idx)
{
	...
	if (Idx < MLME_TASK_EXEC_MULTIPLE) {
		pAd->RalinkCounters.FalseCCACnt_100MS[Idx] =
		    RxStaCnt1.field.FalseCca;
		pAd->RalinkCounters.PLCPErrCnt_100MS[Idx] =
		    RxStaCnt1.field.PlcpErr;
	}
}
