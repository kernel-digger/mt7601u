...
UINT8 GetRegulatoryMaxTxPwr(IN PRTMP_ADAPTER pAd, IN UINT8 channel)
{
	...
	for (RegulatoryClassLoop = 0;
		...
		RegulatoryClass =
		    pAd->CommonCfg.RegulatoryClass[RegulatoryClassLoop];
		if (RegulatoryClass >= MaxRegulatoryClassNum) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s: %c%c Unknow Requlatory class (%d)\n",
				  __FUNCTION__, pCountry[0], pCountry[1],
				  RegulatoryClass));
			return 0xff;
		}
...
static VOID PeerTpcReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
	...
	RealRssi =
	    RTMPMaxRssi(pAd,
			ConvertToRssi(pAd, Elem->Rssi0, RSSI_0, Elem->AntSel,
				      BW_20), ConvertToRssi(pAd, Elem->Rssi1,
							    RSSI_1,
							    Elem->AntSel,
							    BW_20),
			ConvertToRssi(pAd, Elem->Rssi2, RSSI_2, Elem->AntSel,
				      BW_20));
		...
		if ((pEntry = TpcReqLookUp(pAd, DialogToken)) != NULL) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s: DialogToken=%x, TxPwr=%d, LinkMargin=%d\n",
				  __FUNCTION__, DialogToken, TpcRepInfo.TxPwr,
	...
	case SPEC_CHANNEL_SWITCH:
...
#ifdef DOT11N_DRAFT3
		{
			...
			if ((Elem->Msg[LENGTH_802_11 + 2] ==
				...
				ChannelSwitchAction(pAd, Elem->Wcid,
