			  ...
			  IN MAC_TABLE_ENTRY * pEntryPeer, OUT UCHAR * pWinSize)
{
	...
	if (CLIENT_STATUS_TEST_FLAG(pEntryPeer, fCLIENT_STATUS_RALINK_CHIPSET))
		...
		MaxPeerRxSize =
		    (((1 << (pEntryPeer->MaxRAmpduFactor + 3)) * 10) / 16) - 1;
	...
	if ((Idx != 0) && (pBAEntry->TID == TID)
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s():TXBAbitmap=%x, BAWinSize=%d, TimeOut=%ld\n",
			  __FUNCTION__, pEntry->TXBAbitmap, pBAEntry->BAWinSize,
	...
	if ((pBAEntry->ORI_BA_Status == Originator_WaitRes)
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			if (INFRA_ON(pAd) &&
				...
				RTMPSetTimer(&pBAEntry->ORIBATimer,
...
VOID PeerAddBAReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM * Elem)
{
...
#ifdef UNALIGNMENT_SUPPORT
	{
		...
		NdisMoveMemory((PUCHAR) (&tmpBaParm),
			       (PUCHAR) (&ADDframe.BaParm), sizeof(BA_PARM));
		*(USHORT *) (&tmpBaParm) = cpu2le16(*(USHORT *) (&tmpBaParm));
		NdisMoveMemory((PUCHAR) (&ADDframe.BaParm),
			   ...
			   IN RX_BLK * pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	if (!RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU)
		...
		if (err_size > 20) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("AMPDU DataSize = %d\n", pRxBlk->DataSize));
			hex_dump("802.11 Header", (UCHAR *) pRxBlk->pHeader,
				 24);
				    ...
				    IN RX_BLK * pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	if (Wcid < MAX_LEN_OF_MAC_TABLE) {
		...
		if (Idx == 0) {
			/* Rec BA Session had been torn down */
			INDICATE_LEGACY_OR_AMSDU_HDR_TRNS(pAd, pRxBlk,
							  FromWhichBSSID);
			return;
		}
		pBAEntry = &pAd->BATable.BARecEntry[Idx];
	...
	/* IV. Receive Sequence within Window Size */
		 ...
		 (Sequence,
		  (((pBAEntry->LastIndSeq + pBAEntry->BAWinSize + 1)) & MAXSEQ),
		  MAXSEQ)) {
		ba_enqueue_reordering_packet_hdr_trns(pAd, pBAEntry, pRxBlk,
						      FromWhichBSSID);
	}
