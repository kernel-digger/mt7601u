...
========================================================================
*/
INT RtmpIoctl_rt_ioctl_giwname(IN RTMP_ADAPTER * pAd,
			 ...
			 IN USHORT subcmd, IN VOID * pData, IN ULONG Data)
{
	...
	case CMD_RTPRIV_IOCTL_TASK_LIST_GET:
		/* get all Tasks */
		{
			RT_CMD_WAIT_QUEUE_LIST *pList =
			    (RT_CMD_WAIT_QUEUE_LIST *) pData;
...
//#ifdef USB_SUPPORT_SELECTIVE_SUSPEND
	...
	case CMD_RTPRIV_IOCTL_ADAPTER_SEND_DISSASSOCIATE:
		...
		if (INFRA_ON(pAd) &&
			...
			os_alloc_mem(NULL, (UCHAR **) & MsgElem,
...
//#endif /* CONFIG_PM */        
	...
	case CMD_RTPRIV_IOCTL_SANITY_CHECK:
		...
		if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE))
			...
			if (pData == NULL
			    || RT_isLegalCmdBeforeInfUp((PSTRING) pData) ==
			    FALSE)
				return NDIS_STATUS_FAILURE;
		}
		break;
	...
	case CMD_RTPRIV_IOCTL_USB_CONFIG_INIT:
		{
			...
			for (i = 0; i < 6; i++)
				pAd->BulkOutEpAddr[i] =
				    pConfig->BulkOutEpAddr[i];

			for (i = 0; i < 6; i++) {
				DBGPRINT(RT_DEBUG_OFF,
					 ("%s():pAd->BulkOutEpAddr=0x%x\n",
	...
	case CMD_RTPRIV_IOCTL_INF_PPA_EXIT:
		if (ppa_hook_directpath_register_dev_fn
			...
			UINT status;
			status =
			    ppa_hook_directpath_register_dev_fn(&pAd->g_if_id,
								pAd->net_dev,
								NULL, 0);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("unregister PPA:g_if_id=%d status=%d\n",
				  pAd->g_if_id, status));
		}
	...
	case CMD_RTPRIV_IOCTL_INF_IW_STATUS_GET:
		/* get wireless statistics */
		{
			...
			if (!RTMP_TEST_FLAG
			    (pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE))
				return NDIS_STATUS_FAILURE;
...
INT Set_MO_FalseCCATh_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("%s: set falseCCA threshold %lu for microwave oven application!!\n",
		  __FUNCTION__, th));
