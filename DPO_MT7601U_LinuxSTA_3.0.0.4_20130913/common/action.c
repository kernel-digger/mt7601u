			    ...
			    OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(S, ACT_IDLE, MT2_PEER_DLS_CATE,
			      (STATE_MACHINE_FUNC) PeerDLSAction);
	...
	if (MlmeAddBAReqSanity(pAd, Elem->Msg, Elem->MsgLen, Addr) &&
...
#ifdef UNALIGNMENT_SUPPORT
		{
			...
			NdisMoveMemory((PUCHAR) (&tmpBaParm),
				       ...
				       sizeof(BA_PARM));
			*(USHORT *) (&tmpBaParm) =
			    cpu2le16(*(USHORT *) (&tmpBaParm));
			NdisMoveMemory((PUCHAR) (&Frame.BaParm),
...
#ifdef CONFIG_STA_SUPPORT
VOID StaPublicAction(IN PRTMP_ADAPTER pAd, IN BSS_2040_COEXIST_IE * pBssCoexIE)
{
...
ULONG BuildIntolerantChannelRep(IN PRTMP_ADAPTER pAd, IN PUCHAR pDest)
{
	...
	for (k = 0; k < 7; k++) {
		...
		for (i = 0; i < pAd->CommonCfg.TriggerEventTab.EventANo; i++) {
			if (pAd->CommonCfg.TriggerEventTab.EventA[i].bValid ==
			    TRUE) {
				if (pAd->CommonCfg.TriggerEventTab.
				    EventA[i].RegClass == TmpRegClass) {
					for (j = 0; j < idx; j++) {
						if (ChannelList[j] ==
						    (UCHAR) pAd->
						    CommonCfg.TriggerEventTab.
						    EventA[i].Channel)
							break;
					}
					if ((j == idx)) {
					...
					pAd->CommonCfg.
					    TriggerEventTab.EventA[i].bValid =
					    FALSE;
				}
				DBGPRINT(RT_DEBUG_ERROR,
		...
		if (idx > 0) {
			...
			*(pDest + ReadOffset + 2) = TmpRegClass;	/* Len = RegClass byte + channel byte. */
			RTMPMoveMemory(pDest + ReadOffset + 3, ChannelList,
				       idx);
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("ACT-BuildIntolerantChannelRep(Size=%ld)\n", FrameLen));
				     ...
				     IN BOOLEAN bAddIntolerantCha)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("ACT - Update2040CoexistFrameAndNotify. BSSCoexist2040 = %x. EventANo = %d. \n",
			   ...
			   IN UCHAR Wcid, IN BOOLEAN bAddIntolerantCha)
{
	...
	if (bAddIntolerantCha == TRUE)
		IntolerantChaRepLen =
		    BuildIntolerantChannelRep(pAd, pOutBuffer + FrameLen);
	...
	case ACTION_BSS_2040_COEXIST:	/* Format defined in IEEE 7.4.7a.1 in 11n Draf3.03 */
		{
			...
			hex_dump("CoexistenceMgmtFrame", Elem->Msg,
				 Elem->MsgLen);
			...
			if (Elem->MsgLen >=
				...
				pIntolerantReport =
				    (BSS_2040_INTOLERANT_CH_REPORT *) ((PUCHAR)
								       pCoexistInfo
								       +
								       sizeof
								       (BSS_2040_COEXIST_ELEMENT));
			...
			if (pAd->CommonCfg.bBssCoexEnable == FALSE
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("20/40 BSS CoexMgmt=%d, bForty_Mhz_Intolerant=%d, ignore this action!!\n",
					  ...
					  pAd->
					  CommonCfg.bForty_Mhz_Intolerant));
				break;
			}

			pBssCoexistIe =
			    (BSS_2040_COEXIST_IE
			     *) (&pCoexistInfo->BssCoexistIe);
	...
	case NOTIFY_BW_ACTION:
		...
		if (Elem->Msg[LENGTH_802_11 + 2] == 0)	/* 7.4.8.2. if value is 1, keep the same as supported channel bandwidth. */
			...
			pEntry->HTPhyMode.field.BW =
			    pEntry->MaxHTPhyMode.field.BW & pAd->
			    CommonCfg.HtCapability.HtCapInfo.ChannelWidth;
		}
...
/*	USHORT			Sequence;*/
	...
	for (i = 1; ((i < MAX_LEN_OF_BA_ORI_TABLE) && (total > 0)); i++) {
		if (pAd->BATable.BAOriEntry[i].ORI_BA_Status == Originator_Done) {
			pEntry =
			    &pAd->MacTab.Content[pAd->BATable.
						 BAOriEntry[i].Wcid];
