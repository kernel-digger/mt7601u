				 ...
				 IN ULONG MsgLen, OUT PUCHAR pAddr2)
{
...
#ifdef UNALIGNMENT_SUPPORT
	{
		...
		NdisMoveMemory((PUCHAR) (&tmpBaParm),
			       (PUCHAR) (&pAddFrame->BaParm), sizeof(BA_PARM));
		*(USHORT *) (&tmpBaParm) = cpu2le16(*(USHORT *) (&tmpBaParm));
		NdisMoveMemory((PUCHAR) (&pAddFrame->BaParm),
			       (PUCHAR) (&tmpBaParm), sizeof(BA_PARM));
	}
#else
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		if ((*LengthVIE + pEid->Len + 2) >= MAX_VIE_LEN) {
			DBGPRINT(RT_DEBUG_WARN,
				 ("%s() - Variable IEs out of resource [len(=%d) > MAX_VIE_LEN(=%d)]\n",
				  __FUNCTION__, (*LengthVIE + pEid->Len + 2),
				  MAX_VIE_LEN));
			break;
		}
				...
				*(USHORT *) (&pHtCapability->HtCapInfo) =
...
#ifdef UNALIGNMENT_SUPPORT
				{
					...
					NdisMoveMemory((PUCHAR) (&extHtCapInfo),
						       (PUCHAR)
						       (&pHtCapability->ExtHtCapInfo), sizeof(EXT_HT_CAP_INFO));
					*(USHORT *) (&extHtCapInfo) =
					    cpu2le16(*(USHORT *)
						     (&extHtCapInfo));
					NdisMoveMemory((PUCHAR)
						       (&pHtCapability->ExtHtCapInfo), (PUCHAR) (&extHtCapInfo), sizeof(EXT_HT_CAP_INFO));
				}
#else
		...
		case IE_TIM:
			if (SubType == SUBTYPE_BEACON) {
				if (INFRA_ON(pAd)
				    && NdisEqualMemory(pBssid,
						       pAd->CommonCfg.Bssid,
						       MAC_ADDR_LEN)) {
					GetTimBit((PCHAR) pEid,
						  pAd->StaActive.Aid, &TimLen,
						  pBcastFlag, pDtimCount,
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		case IE_SSID:
			...
			if (Sanity & 0x1)
				break;
			if (pEid->Len <= MAX_LEN_OF_SSID) {
				NdisMoveMemory(&ie_list->Ssid[0], pEid->Octet,
					       pEid->Len);
		...
		case IE_SUPP_RATES:
			if (pEid->Len <= MAX_LEN_OF_SUPPORTED_RATES) {
				Sanity |= 0x2;
				NdisMoveMemory(&ie_list->SupRate[0],
					       pEid->Octet, pEid->Len);
		...
		case IE_HT_CAP:
			if (pEid->Len >= SIZE_HT_CAP_IE) {	/*Note: allow extension.!! */
				NdisMoveMemory(&ie_list->HtCapability,
					       pEid->Octet,
					       sizeof(HT_CAPABILITY_IE));
		...
		case IE_ADD_HT:
			if (pEid->Len >= sizeof(ADD_HT_INFO_IE)) {
				...
				*(USHORT *) (&ie_list->AddHtInfo.AddHtInfo2) =
				    cpu2le16(*(USHORT *)
					     (&ie_list->AddHtInfo.AddHtInfo2));
				*(USHORT *) (&ie_list->AddHtInfo.AddHtInfo3) =
				    cpu2le16(*(USHORT *)
					     (&ie_list->AddHtInfo.AddHtInfo3));
...
#ifdef CONFIG_STA_SUPPORT
				IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
					if (ChannelSanity(pAd, ie_list->Channel)
					    == 0) {
						goto SanityCheck;
					}
				}
		...
		case IE_IBSS_PARM:
			if (pEid->Len == 2) {
				NdisMoveMemory(&ie_list->AtimWin, pEid->Octet,
					       pEid->Len);
			} else {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s() - wrong IE_IBSS_PARM\n",
			...
			/* Check broadcom-defiend pre-802.11nD1.0 OUI for HT related IE, including HT Capatilities IE and HT Information IE */
				...
				if ((pEid->Octet[3] == OUI_PREN_HT_CAP)
					...
					NdisMoveMemory(&ie_list->HtCapability,
						       ...
						       sizeof
						       (HT_CAPABILITY_IE));
					ie_list->PreNHtCapabilityLen =
					    SIZE_HT_CAP_IE;
				}

				if ((pEid->Octet[3] == OUI_PREN_ADD_HT)
				    && (pEid->Len >= 26)) {
					NdisMoveMemory(&ie_list->AddHtInfo,
						       &pEid->Octet[4],
						       sizeof(ADD_HT_INFO_IE));
		...
		case IE_EXT_SUPP_RATES:
			if (pEid->Len <= MAX_LEN_OF_SUPPORTED_RATES) {
				NdisMoveMemory(&ie_list->ExtRate[0],
					       pEid->Octet, pEid->Len);
		...
		case IE_EXT_CAPABILITY:
			if (pEid->Len >= 1) {
				NdisMoveMemory(&ie_list->ExtCapInfo,
					       &pEid->Octet[0],
		...
		case IE_VHT_CAP:
			if (pEid->Len == sizeof(VHT_CAP_IE)) {
				NdisMoveMemory(&ie_list->vht_cap_ie,
					       &pEid->Octet[0],
					       sizeof(VHT_CAP_IE));
		...
		case IE_VHT_OP:
			if (pEid->Len == sizeof(VHT_OP_IE)) {
				NdisMoveMemory(&ie_list->vht_op_ie,
					       &pEid->Octet[0],
					       sizeof(VHT_OP_IE));
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		case IE_OVERLAPBSS_SCAN_PARM:
			if (pEid->Len == sizeof(OVERLAP_BSS_SCAN_IE)) {
				brc = TRUE;
				RTMPMoveMemory(BssScan, pEid->Octet,
					       sizeof(OVERLAP_BSS_SCAN_IE));
			} else {
				DBGPRINT(RT_DEBUG_TRACE,
...
BOOLEAN PeerDeauthSanity(IN PRTMP_ADAPTER pAd,
			 ...
			 OUT PUCHAR pAddr1,
			 OUT PUCHAR pAddr2,
			 OUT PUCHAR pAddr3, OUT USHORT * pReason)
{
	...
	while (((UCHAR *) eid_ptr + eid_ptr->Len + 1) < ((UCHAR *) Fr + MsgLen)) {
		...
		case IE_SUPP_RATES:
			if ((eid_ptr->Len <= MAX_LEN_OF_SUPPORTED_RATES)
			    && (eid_ptr->Len > 0)) {
				NdisMoveMemory(Rates, eid_ptr->Octet,
					       eid_ptr->Len);
				DBGPRINT(RT_DEBUG_TRACE,
		...
		case IE_EXT_SUPP_RATES:
			if (eid_ptr->Len + *pRatesLen <=
			    MAX_LEN_OF_SUPPORTED_RATES) {
				NdisMoveMemory(&Rates[*pRatesLen],
...
BOOLEAN PeerProbeReqSanity(IN PRTMP_ADAPTER pAd,
			   ...
			   OUT PUCHAR pAddr2,
			   OUT CHAR Ssid[],
			   OUT UCHAR * SsidLen, OUT BOOLEAN * bRssiRequested)
{
	PFRAME_802_11 Fr = (PFRAME_802_11) Msg;
	UCHAR *Ptr;
