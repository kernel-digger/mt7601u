...
struct iw_priv_args privtab[] = {
	...
	{RTPRIV_IOCTL_SHOW, IW_PRIV_TYPE_CHAR | 1024,
...
/*	if (INFRA_ON(pAd)||ADHOC_ON(pAd)) */
	if ((Mode == RTMP_CMD_STA_MODE_INFRA)
	    || (Mode == RTMP_CMD_STA_MODE_ADHOC)) {
...
/*	range->num_channels =  pAd->ChannelListNum; */
	...
	os_alloc_mem(NULL, (UCHAR **) & pChannel,
		     sizeof(UCHAR) * ChannelListNum);
	if (pChannel == NULL)
		return -ENOMEM;
		     ...
		     struct iw_point *dwrq, char *extra)
{
	...
	if (strcasecmp(ext, "START") == 0) {
		...
		RTMP_STA_IoctlHandle(pAd, NULL,
				     CMD_RTPRIV_IOCTL_STA_SIOCSIWPRIVRSSI, 0,
				     &AvgRssi0, 0, dev->priv_flags);
		...
		UCHAR mac[6];
		RTMP_STA_IoctlHandle(pAd, NULL,
				     CMD_RTPRIV_IOCTL_STA_SIOCGIFHWADDR, 0, mac,
				     0, dev->priv_flags);
		snprintf(ext, min(dwrq->length, (UINT16) (strlen(ext) + 1)),
			 "MACADDR = %02x.%02x.%02x.%02x.%02x.%02x", mac[0],
	...
	if (copy_to_user
	    (dwrq->pointer, ext, min(dwrq->length, (UINT16) (strlen(ext) + 1))))
		ret = -EFAULT;
...
static void set_quality(VOID * pAd,
			struct iw_quality *iq, RT_CMD_STA_IOCTL_BSS * pBss)
...
/* because android will set scan and get scan when interface down */
	...
	for (i = 0; i < pIoctlScan->BssNr; i++) {
		...
		memcpy(iwe.u.ap_addr.sa_data, &pIoctlScan->pBssTable[i].Bssid,
		       ETH_ALEN);
...

		{
			...
			if (pBssEntry->ChannelWidth == 1)
				sprintf(iwe.u.name, "%s %s", iwe.u.name,
					"BW40");
			else
				sprintf(iwe.u.name, "%s %s", iwe.u.name,
					"BW20");
		...
		current_ev =
		    IWE_STREAM_ADD_POINT(info, current_ev, end_buf, &iwe,
					 (PSTRING) pIoctlScan->pBssTable[i].
					 Ssid);
		if (current_ev == previous_ev) {
#if WIRELESS_EXT >= 17
			status = -E2BIG;
		...
		} else if (pIoctlScan->pBssTable[i].BssType ==
...
/*			MAP_CHANNEL_ID_TO_KHZ(ch, m); */
			RTMP_STA_IoctlHandle(pAd, NULL,
					     CMD_RTPRIV_IOCTL_CHID_2_FREQ, 0,
					     (VOID *) & m, ch,
					     RT_DEV_PRIV_FLAGS_GET(dev));
			iwe.u.freq.m = m * 100;
			iwe.u.freq.e = 1;
	...
	/*================================ */
		...
		current_ev =
		    IWE_STREAM_ADD_POINT(info, current_ev, end_buf, &iwe,
					 (char *)pIoctlScan->
					 MainSharedKey[(iwe.u.data.
							flags & IW_ENCODE_INDEX)
						       - 1]);
		if (current_ev == previous_ev) {
#if WIRELESS_EXT >= 17
			status = -E2BIG;
		...
		if (pIoctlScan->pBssTable[i].SupRateLen) {
			UCHAR tmpRate =
			    pIoctlScan->pBssTable[i].SupRate[pIoctlScan->
							     pBssTable[i].
							     SupRateLen - 1];
			memset(&iwe, 0, sizeof(iwe));
			iwe.cmd = SIOCGIWRATE;
			...
			if (pIoctlScan->pBssTable[i].ExtRateLen) {
				UCHAR tmpSupRate =
				    (pIoctlScan->pBssTable[i].
				     SupRate[pIoctlScan->pBssTable[i].
					     SupRateLen - 1] & 0x7f);
				UCHAR tmpExtRate =
				    (pIoctlScan->pBssTable[i].
				     ExtRate[pIoctlScan->pBssTable[i].
					     ExtRateLen - 1] & 0x7f);
				iwe.u.bitrate.value =
				    (tmpSupRate >
				     tmpExtRate) ? (tmpSupRate) *
				    500000 : (tmpExtRate) * 500000;
				tmpRate =
				    (tmpSupRate >
				     tmpExtRate) ? tmpSupRate : tmpExtRate;
			}

			if (tmpRate == 0x6c
			    && pIoctlScan->pBssTable[i].HtCapabilityLen > 0) {
				int rate_count = RT_RateSize / sizeof(__s32);
/*				HT_CAP_INFO capInfo = pIoctlScan->pBssTable[i].HtCapability.HtCapInfo; */
				int shortGI =
				    pIoctlScan->pBssTable[i].
				    ChannelWidth ? pIoctlScan->pBssTable[i].
				    ShortGIfor40 : pIoctlScan->pBssTable[i].
				    ShortGIfor20;
				int maxMCS =
				    pIoctlScan->pBssTable[i].MCSSet ? 15 : 7;
				int rate_index =
				    12 +
				    ((UCHAR) pIoctlScan->pBssTable[i].
				     ChannelWidth * 24) +
		...
		if (pIoctlScan->pBssTable[i].WpaIeLen > 0) {
			...
			iwe.cmd = IWEVCUSTOM;
			iwe.u.data.length =
			    (pIoctlScan->pBssTable[i].WpaIeLen * 2) + 7;
			NdisMoveMemory(custom, "wpa_ie=", 7);
			for (idx = 0; idx < pIoctlScan->pBssTable[i].WpaIeLen;
			     idx++)
		...
		if (pIoctlScan->pBssTable[i].RsnIeLen > 0) {
			...
			iwe.cmd = IWEVCUSTOM;
			iwe.u.data.length =
			    (pIoctlScan->pBssTable[i].RsnIeLen * 2) + 7;
			NdisMoveMemory(custom, "rsn_ie=", 7);
			for (idx = 0; idx < pIoctlScan->pBssTable[i].RsnIeLen;
			     idx++)
				sprintf(custom, "%s%02x", custom,
...
/*	POS_COOKIE pObj; */
	...
	/*check if the interface is down */
/*    	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	if (RTMP_DRIVER_IOCTL_SANITY_CHECK(pAd, this_char) !=
	    NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_TRACE, ("INFO::Network is down!\n"));
		return -ENETDOWN;
	} else {
...
/*    if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	RTMP_STA_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_STA_IW_GET_STATISTICS,
			     0, extra, IW_PRIV_SIZE_MASK,
			     RT_DEV_PRIV_FLAGS_GET(dev));
...
/*    if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	RTMP_STA_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_STA_SIOCSIWMLME,
			     Subcmd, NULL, pMlme->reason_code,
			     RT_DEV_PRIV_FLAGS_GET(dev));
	return 0;
}
#endif /* SIOCSIWMLME */
...
/*	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	case IW_AUTH_RX_UNENCRYPTED_EAPOL:
		pIoctlWpa->flags =
		    RT_CMD_STA_IOCTL_WPA_AUTH_RX_UNENCRYPTED_EAPOL;
		break;
	...
	case IW_AUTH_80211_AUTH_ALG:
		...
		if (param->value & IW_AUTH_ALG_SHARED_KEY)
			pIoctlWpa->value =
			    RT_CMD_STA_IOCTL_WPA_AUTH_80211_AUTH_ALG_SHARED;
		else if (param->value & IW_AUTH_ALG_OPEN_SYSTEM)
			pIoctlWpa->value =
			    RT_CMD_STA_IOCTL_WPA_AUTH_80211_AUTH_ALG_OPEN;
		else if (param->value & IW_AUTH_ALG_LEAP)
			pIoctlWpa->value =
			    RT_CMD_STA_IOCTL_WPA_AUTH_80211_AUTH_ALG_LEAP;
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case IW_AUTH_TKIP_COUNTERMEASURES:
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s::IW_AUTH_TKIP_COUNTERMEASURES -(param->value = %d)\n",
			  __FUNCTION__, param->value));
		break;
	default:
		return -EOPNOTSUPP;
...
/*	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	case IW_AUTH_80211_AUTH_ALG:
		param->value =
		    (pIoctlWpa->value ==
		     0) ? IW_AUTH_ALG_SHARED_KEY : IW_AUTH_ALG_OPEN_SYSTEM;
		break;
...
/*	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	if (RTMP_STA_IoctlHandle
	    (pAd, NULL, CMD_RTPRIV_IOCTL_STA_SIOCSIWENCODEEXT, 0, pIoctlSec, 0,
...
/*    if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	if (RTMP_STA_IoctlHandle
	    (pAd, NULL, CMD_RTPRIV_IOCTL_STA_SIOCGIWENCODEEXT, 0, pIoctlSec, 0,
	     RT_DEV_PRIV_FLAGS_GET(dev)) != NDIS_STATUS_SUCCESS) {
		ext->key_len = 0;
		RT_CMD_STATUS_TRANSLATE(pIoctlSec->Status);
		return pIoctlSec->Status;
...
/*    if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	if (RTMP_STA_IoctlHandle
	    (pAd, NULL, CMD_RTPRIV_IOCTL_STA_SIOCSIWGENIE, 0, extra,
	     wrqu->data.length,
	     RT_DEV_PRIV_FLAGS_GET(dev)) != NDIS_STATUS_SUCCESS)
		return -EINVAL;
	else
...
/*    if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	if (RTMP_STA_IoctlHandle
	    (pAd, NULL, CMD_RTPRIV_IOCTL_STA_SIOCGIWGENIE, 0, pIoctlRsnIe, 0,
	     RT_DEV_PRIV_FLAGS_GET(dev)) != NDIS_STATUS_SUCCESS)
		return -E2BIG;
...
/*			memcpy(wrqin->u.name, pAd->CurrentAddress, ETH_ALEN); */
		RTMP_STA_IoctlHandle(pAd, NULL,
				     CMD_RTPRIV_IOCTL_STA_SIOCGIFHWADDR, 0,
				     wrqin->u.name, 0,
	...
	case SIOCGIWNICKN:	/*get node name/nickname */
		{
			RT_CMD_STA_IOCTL_NICK_NAME NickName, *pNickName =
			    &NickName;
			...
			Status =
			    copy_to_user(erq->pointer, nickname, erq->length);
			break;
		}
	...
	case RT_PRIV_IOCTL_EXT:
		...
		Status =
		    RTMP_STA_IoctlHandle(pAd, wrq, CMD_RT_PRIV_IOCTL, subcmd,
					 NULL, 0,
					 RT_DEV_PRIV_FLAGS_GET(net_dev));
		break;
	case SIOCGIWPRIV:
		if (wrqin->u.data.pointer) {
			if (access_ok
			    (VERIFY_WRITE, wrqin->u.data.pointer,
			     sizeof(privtab)) != TRUE)
				break;
			if ((sizeof(privtab) / sizeof(privtab[0])) <=
			    wrq->u.data.length) {
				wrqin->u.data.length =
				    sizeof(privtab) / sizeof(privtab[0]);
				if (copy_to_user
				    (wrqin->u.data.pointer, privtab,
				     sizeof(privtab)))
					Status = -EFAULT;
			} else
				Status = -E2BIG;
	...
	case RTPRIV_IOCTL_SET:
		if (access_ok
		    (VERIFY_READ, wrqin->u.data.pointer,
		     wrqin->u.data.length) != TRUE)
			break;
		return rt_ioctl_setparam(net_dev, NULL, NULL,
					 wrqin->u.data.pointer);
		break;
	case RTPRIV_IOCTL_GSITESURVEY:
		RTMP_STA_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_SITESURVEY_GET,
				     0, NULL, 0,
				     RT_DEV_PRIV_FLAGS_GET(net_dev));
