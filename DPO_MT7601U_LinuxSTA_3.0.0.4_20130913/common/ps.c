...
========================================================================
*/
NDIS_STATUS RtmpInsertPsQueue(IN PRTMP_ADAPTER pAd,
			      ...
			      IN MAC_TABLE_ENTRY * pMacEntry, IN UCHAR QueIdx)
{
...
#endif /* UAPSD_SUPPORT */
	{
		if (pMacEntry->PsQueue.Number >= MAX_PACKETS_IN_PS_QUEUE) {
			...
			InsertTailQueue(&pMacEntry->PsQueue,
...
/*
  ...
  ========================================================================
*/
VOID RtmpHandleRxPsPoll(IN PRTMP_ADAPTER pAd,
		...
		if (isActive == FALSE) {
			if (pMacEntry->PsQueue.Head) {
				...
				if (pMacEntry->PsQueue.Number >= 1)
					RTMP_SET_PACKET_MOREDATA(RTPKT_TO_OSPKT
								 (pEntry),
								 TRUE);
				...
				if (NumOfOldPsPkt !=
					...
					if (RTMP_GET_PACKET_DHCP
					    (RTPKT_TO_OSPKT(pEntry))
					    ||
					    RTMP_GET_PACKET_EAPOL(RTPKT_TO_OSPKT
								  (pEntry))
					    ||
					    RTMP_GET_PACKET_WAI(RTPKT_TO_OSPKT
								(pEntry))) {
						/*
				...
				if (CLIENT_STATUS_TEST_FLAG
				    (pMacEntry, fCLIENT_STATUS_WMM_CAPABLE))
					bQosNull = TRUE;

				RtmpEnqueueNullFrame(pAd, pMacEntry->Addr,
						     pMacEntry->CurrTxRate, Aid,
						     pMacEntry->apidx, bQosNull,
						     TRUE, 0);
			}
		} else {
#ifdef UAPSD_SUPPORT
...
/*					break; */
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("rcv PS-POLL (AID=%d not match) from %02x:%02x:%02x:%02x:%02x:%02x\n",
			  Aid, pAddr[0], pAddr[1], pAddr[2], pAddr[3], pAddr[4],
			  pAddr[5]));
...
/*
	...
	==========================================================================
 */
BOOLEAN RtmpPsIndicate(IN PRTMP_ADAPTER pAd,
...
VOID RtmpPsActiveExtendCheck(IN PRTMP_ADAPTER pAd)
{
	/* count down the TDLS active counter */
}

VOID RtmpPsModeChange(IN PRTMP_ADAPTER pAd, IN UINT32 PsMode)
{
