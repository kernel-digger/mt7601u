...
VOID CarrierDetectionPeriodicStateCtrl(IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->CommonCfg.CarrierDetect.Debug == RT_DEBUG_TRACE) {
		...
		RTUSBMultiRead(pAd, 0x4CB0, (PUCHAR) & CarrDetectDebug,
			       sizeof(CarrDetectDebug));
		printk("delta_div = 0x%02X, rRadarToneCount = %u, Recheck = %u, Criteria = %u, Threshold = 0x%08X, VGA_Mask = 0x%04X\n", CarrDetectDebug.delta_div, CarrDetectDebug.RadarToneCount, CarrDetectDebug.ReCheck, CarrDetectDebug.Criteria << 6,	/* ms -> 16us */
		       CarrDetectDebug.Threshold, CarrDetectDebug.VGA_Mask);
	}

	RTUSBMultiRead(pAd, RADAR_TONE_COUNT, (PUCHAR) & CarrDetectParam,
		       sizeof(CarrDetectParam));
	switch (*pCD_State) {
	case CD_NORMAL:
...
#ifdef CARRIER_DETECTION_FIRMWARE_SUPPORT
	RTMP_IO_WRITE8(pAd, CD_CHECK_COUNT,
		       pAd->CommonCfg.CarrierDetect.recheck1);
	/* send enable cmd to mcu to take effect */
...
VOID CarrierDetectionStart(PRTMP_ADAPTER pAd)
{
	...
	if (pAd->CommonCfg.CarrierDetect.Enable) {
...
#ifdef CARRIER_DETECTION_FIRMWARE_SUPPORT
		{
			...
			RTUSBMultiWrite(pAd, CD_CRITERIA, (PUCHAR) & criteria,
					2, FALSE);
			RTMP_IO_WRITE8(pAd, CD_CHECK_COUNT,
				       pAd->CommonCfg.CarrierDetect.recheck1);
			AsicSendCommandToMcu(pAd, CD_ONOFF_MCU_CMD, 0xff, 0x01,
					     0x00, FALSE);
		}
#else
...
VOID ToneRadarProgram_v2(PRTMP_ADAPTER pAd, ULONG threshold)
{
	...
	bbp = pAd->CommonCfg.CarrierDetect.delta | ((pAd->CommonCfg.CarrierDetect.SymRund & 0x3) << 4) | ((pAd->CommonCfg.CarrierDetect.div_flag & 0x1) << 6) | 0x80;	/* Full 40MHz Detection Mode */
